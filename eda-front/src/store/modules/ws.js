...
const actions = {
    connectWSAndGetInfo({ commit }) {
        ...
        if (userId) {
            ...
            const limit = 10;
            console.log('测试连接');
            const msgUrl1 = `ws://${host}/wsmsg/ws/${appName}/${userId}/${limit}`

            const msgUrl2 = `ws://172.18.0.185:30420/ws/${appName}/${userId}/${limit}`    // 本地测试
            const msgUrl = process.env.NODE_ENV === 'development' ? msgUrl2 : msgUrl1 
            ...
            wsInit(msgWS, (value) => {
                ...
                return new Promise((resolve, reject) => {
                    queryUnreadCount().then(response => {
    ...
    connectWSAndGetAssisance({ commit }) {
        ...
        if (userId) {
            ...
            const msgUrl1 = `ws://${host}/wsmsg/ws/${appName}/${userId}/${0}`
            const msgUrl2 = `ws://172.18.0.185:30478/ws/${appName}/${userId}/${0}`    // 本地测试
            const msgUrl = process.env.NODE_ENV === 'development' ? msgUrl2 : msgUrl1 
