        ...
        <el-card>
            <div class="flex justify-between">
                ...
                <div class="flex flex-column justify-between">
                    <div>
                        <el-button type="warning" size="mini" icon="el-icon-edit">修改信息</el-button>
                    </div>
                    <div>
                        <el-button type="danger" size="mini" icon="el-icon-plus">删除项目</el-button>
        ...
        <el-tabs class="el-tab-edu" v-model="activeName" @tab-click="handleClick">
                        ...
                        <div class="" style="padding: 10px 0 0px 10px;">
                            <el-form ref="queryEnvFormRef" :model="queryEnvParams" :inline="true">
                                <el-form-item label="设计云名称" prop="vmName">
                                    <el-input v-model="queryEnvParams.vmName" id="vmName" placeholder="请输入设计云名称" clearable
                    ...
                    <el-card class="marginTop10">
                        <template #header>
                            <el-button :disabled="vmsList && vmsList.length > 0" type="primary" icon="el-icon-plus"
                                @click="goToDetail">创建设计云环境</el-button>
                            &nbsp;<el-button :disabled="!(vmsList && vmsList.length > 0)" type="primary" icon="el-icon-plus"
                                @click="handleAddUser">手动创建</el-button>
                            <el-button style="float: right;"
                                :disabled="!(multipleVmSelection && multipleVmSelection.length > 0)" type="primary"
                                icon="el-icon-edit" @click="handleVms">批量修改</el-button>
                        </template>
                        <el-table :data="vmsList" style="width: 100%" v-loading="loading"
                            ...
                            <el-table-column prop="vmId" label="设计云ID" width="100" align="center">
                            ...
                            <el-table-column prop="vmName" label="设计云名称" min-width="180" align="center">
                                <template #default="scope">
                                    <!-- <el-button type="primary" size="small" @click.stop="handleShowDetail(scope.row)">查看详情</el-button> -->
                                    <a v-if="scope.row.status == '0'" @click="goToEnvDetail(scope.row.vmId)"
                                        class="primaryColor">{{
                                            scope.row.vmName }}
                            ...
                            <el-table-column prop="hostname" label="主机名称" min-width="180" align="center"
                                show-overflow-tooltip>
                            </el-table-column>
                            <el-table-column prop="powerState" label="电源状态" width="180" align="center">
                                <!-- <template #default="scope">
                            ...
                            <el-table-column prop="userCountsIds" label="使用人数" width="100" align="center">
                                <template #default="scope">
                            ...
                            <el-table-column label="操作" align="center" min-width="250" fixed="right">
                                <template #default="scope">
                                    <div v-if="scope.row.status == '0'">
                                        ...
                                        <el-button type="info" class="editInfo" size="small"
                                            v-if="scope.row.powerState == 'poweredOn'" @click.stop="reboot(scope.row)"
                                            plain>重启</el-button>
                                        ...
                                        <el-button size="small" type="primary" slot="reference"
                                            @click="handleUpdate(scope.row)" class="editPrimary">修改</el-button>
                                        <el-divider direction="vertical"></el-divider>
                        ...
                        <pagination v-show="total > 0" :total="total" :page.sync="listQuery.page"
                            :limit.sync="listQuery.limit" @pagination="searchQuery" />
                    </el-card>
                </div>
            </el-tab-pane>
            ...
            <el-tab-pane label="项目资料" name="second">
                <div v-if="activeName == 'second'">
                    <el-card>
                        ...
                        <el-table :data="labsList" style="width: 100%">
                            ...
                            <el-table-column label="操作" align="center" min-width="180" fixed="right">
                                <template #default="scope">
                                    <el-button :disabled="!isfileTypesCanShow[scope.row.fileSuffix]" size="small"
                                        type="success" class="editSuccess" slot="reference"
                                    ...
                                    <el-divider direction="vertical"></el-divider>
                                    <el-popconfirm title="确定删除吗？" @onConfirm="handleDeleteCourseSource(scope.row)"
                                        onCancel="">
                    ...
                    <upload-file-dialog :updateFileInfo="updateFileInfo" :dialogStatus="dialogStatus"
                        :loadding="fileUpLoading" ref="upload-file-dialog" @doUpload="doUpload" @handleClose="handleClose"
...
export default {
    ...
    name: "courseMng",
    components: {
        Pagination, FileList, StudentList, BorderContainer, UploadFileDialog,
        HomeWorkDialog, CourseMngCom, CourseLab
    },
    props: {},
    data() {
        return {
    ...
    methods: {
        ...
        handleDeleteCourse() {
            ...
            this.$confirm('删除课程后会同步删除设计云环境及其所有数据，无法恢复，请慎重选择！', {
        ...
        //
        handleDownload(row) {
            fetch(row.filePath).then(res => res.blob()).then(blob => { // 将链接地址字符内容转变成blob地址
        ...
        async handleDelete(row) {
            ...
            let msg = "删除设计云会同步删除该设计云中的所有数据，无法恢复，请慎重选择！"
            if (index != -1) {
                userList = userCountsIds && userCountsIds.split(',')
                userList && userList.length > 0 && userList.map(i => {
                    if (userId == i) {
                        msg = "该设计云包含当前老师账号，若删除该设计云，该设计云中的所有数据将全部删除，无法恢复，请慎重选择！"
                        return
                    }
                })
            }
        ...
        createVm() {
            let courseId = this.courseId
            this.$router.push({ path: '/classMng/createEnvironment', query: { courseId } });
        },
        ...
        async handleDoM(e, index) {
            ...
            let courseId = this.courseId
            const res1 = await getHomeworkStatistics({ courseId, homeworkId: this.homeworkId })
        ...
        submitFormHomeWork(form) {
            ...
            if (this.dialog.status == 'update') {
                ...
                editHomework(form).then((reponse) => {
        ...
        handleCorrention(obj) {
            ...
            const { id, paperId, userId, userName, className, userAccount, startTime, endTime } = obj && obj.row
            const { page, limit } = this.listHomeworkQuery
            let total = this.homewrkTotal
            $index = $index + 1;
            let courseId = this.courseId;
            let objs = { $index, page, limit, total, id, paperId, userId, userName, homeworkId, courseId, courseName, className, userAccount, startTime, endTime }

            objs = JSON.stringify(objs);
            objs = encodeURI(objs);
            this.$router.push({ path: '/classMng/correctionHomeWork', query: { obj: objs } })
        },
