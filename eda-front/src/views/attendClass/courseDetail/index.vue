<template>
    <div class="app-container">
        <border-container class="marginBottom10 border-container" :title="courseObj.name" :height="15" :isBgShow="false">
            <template #content>
                <div class="font16 flex justify-between" style="padding:0 10px">
                    <div class="flex marginTop10" style="padding-left:2px">
                        <div class="marginRt20 title-decration2">课程ID：{{ courseObj.courseNum || "-" }}</div>
                        <div class="marginRt20 title-decration2">学年：{{ courseObj.academicYear || '-' }}</div>
                        <div class="marginRt20 title-decration2">课程人数：{{ courseObj.studentNum || 0 }}人</div>
                        <div class="marginRt20 title-decration2">创建时间：{{ courseObj.createAt }}</div>
                    </div>
                </div>
            </template>
        </border-container>
        ...
        <el-tabs v-model="activeName" class="el-tab-edu" @tab-click="handleClick">
            ...
            <el-tab-pane label="课程学习" name="fourth">
                <course-mng-com ref="course-mng-com" :isStudent="true" :courseId="courseId"></course-mng-com>
            </el-tab-pane>
                ...
                <div v-if="activeName == 'first'">
                            ...
                            <div class="" style="padding: 20px 0 10px 10px;">
                                <el-form ref="queryFormRef" :model="queryParams" :inline="true">
                                    <el-form-item label="作业名称" prop="homeworkName">
                                        <el-input v-model="queryParams.homeworkName" placeholder="请输入作业名称"
                    ...
                    <el-card class="marginTop10">
                        <el-table :data="homeworksList" style="width: 100%" row-key="id">
                            ...
                            <el-table-column prop="homeworkName" label="作业名称" align="center" width="200"
                                show-overflow-tooltip>
                            </el-table-column>
                            <el-table-column prop="title" label="试卷名称" align="center" width="280" show-overflow-tooltip>

                            </el-table-column>
                            <el-table-column prop="endTime" label="截止日期" align="center" width="180"></el-table-column>

                            <el-table-column prop="completionStatus" label="作业状态" align="center" width="160">
                                <template #default="scope">
                                    <el-tag type="danger" v-if="scope.row.completionStatus == 0">未提交</el-tag>
                                    <el-tag type="warning" v-else-if="scope.row.completionStatus == 1">待批改</el-tag>
                                    <el-tag type="success" v-else>已完成</el-tag>
                                </template>
                            </el-table-column>
                            <!-- <el-table-column prop="submitTime" label="提交时间" align="center" width="180">
                            </el-table-column> -->
                            <el-table-column sortable prop="score" label="评分" align="center" width="120">
                                <template #default="scope">
                            ...
                            <el-table-column prop="suggestion" label="批改建议" align="center"></el-table-column>

                            <el-table-column label="操作" align="center" fixed="right">
                                <template #default="scope">
                                    <el-button v-if="scope.row.completionStatus == 0" type="success" class="editSuccess"
                                        size="small"
                                        @click="gotoPage('courseDetail/doHomeWork', scope.row)">进行作答</el-button>
                                    <el-button v-if="scope.row.completionStatus == 1" type="primary" size="small"
                                        :disabled="true">待批改</el-button>
                                    <el-button v-if="scope.row.completionStatus == 2" type="success" class="editSuccess"
                                        size="small" @click="openHomework(scope.row)">批改详情</el-button>
                                </template>
                            </el-table-column>
                        </el-table>
                        <!-- 作废 -->
                        ...
                        <el-dialog :title="fileDialog.title" :visible.sync="fileDialog.visible" @close="cancelfileDialog"
                            ...
                            <div class="dialog-paper-container">
                                <el-form ref="fileForm" :model="questionForm">
                                    <paper-container :isWatch="false" :questionForm="questionForm"></paper-container>
                                </el-form>
                            </div>
                    ...
                    <el-card>
                        <el-table :data="labsList" style="width: 100%">
                            ...
                            <el-table-column label="操作" align="center" min-width="180" fixed="right">
                                <template #default="scope">
                                    <el-button size="small" type="success" class="editSuccess" slot="reference"
        ...
        <el-dialog :title="textMap[dialog.status]" :visible.sync="dialog.visible" @close="cancel" class="dialog"
            ...
            <el-form ref="form" :model="form" :rules="rules" label-width="80px">
                <el-form-item label="作业名称" prop="homeworkName">
                    <el-input v-model="form.homeworkName" placeholder="请输入作业名称" style="width:300px"></el-input>
                </el-form-item>
            </el-form>
    ...
    methods: {
        ...
        handleFilesList1(filesList) {
            filesList && filesList.length > 0 && filesList.map(i => {
                const { editTime, fileName, filePath, fileSize, homeworkMarkId, isDir, userId, vmId } = i;
                if (isDir) {
                    // i.children = [],
                    i.hasChildren = true
                }
