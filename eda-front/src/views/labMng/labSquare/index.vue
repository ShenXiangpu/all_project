<template>
    <div>
        ...
        <el-card class="my-lab-card-body" v-loading="loading">
            <!-- <template #header>
                <div class="flex justify-between">
                    <el-popover placement="right"  trigger="click">
                        <div >
                            <el-button type="primary"  size="small" style="border: none;" class="primaryBgColor" @click="handleAdd('/lab/operationLab')">自定义实验</el-button> 
                            <el-button size="small" class="primaryBgColor" type="primary" style="border: none;" @click="handleAdd('/lab/squareLab')">实验广场添加</el-button>
                        </div>
            ...
            <el-row :gutter="10">
                <el-col class="el-row index-color pointer" :xs="8" :sm="8" :md="8" :lg="6" :xl="5"
                    v-for="item in labList" :key="item.id">
                    <lab-list @handleDetail="handleDetail(item.trialId)" @handleUpdate="handleUpdate(item.trialCourseRelId)"
                        :isShowUpdate="false" :imgUrl="imgUrlList[item.coverImageStage]" :labName="item.coverImageName"
                        ...
                        <template #lab-description>
                            <div class="description-container" @click="handleDetail(item.trialId)">
                                <div class="flex justify-between">
                                ...
                                <div style="display: flex;justify-content:space-between;font-size: 16px;">
                                    <div class="lab-card-bottom" style="width: 60%;">
                                        <span class="view-text" style="display: inline-block;width: 80%;">
                                            <i class="el-icon-s-operation"></i> {{ item.recommendStandard }}</span>
                                    </div>
                                    <div class="lab-card-bottom" style="width: 40%;text-align: right;">
                                        <span><i class="el-icon-s-flag"></i> {{ `${item && item.userCountInfo &&
                                            item.userCountInfo.oneTrialAllCourseFinish || 0}` + '次' }}</span>
                                    </div>
                                </div>
                                <div class="flex justify-start">
                                    <el-popover placement="top" trigger="hover" v-if="item && item.tools">
                                        <div>
                                            <tools-detail :tools="item.tools" v-if="item && item.tools"></tools-detail>
                                        </div>
                                        <el-tag v-if="item && item.tools" slot="reference" type="info" size="mini"
...
export default {
    ...
    name: "myLab",
    components: {
        BorderContainer, LabList, Pagination, LabDetail, ToolsDetail
    },
    props: {
