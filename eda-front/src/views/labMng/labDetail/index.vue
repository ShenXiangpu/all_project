                ...
                <div class="trialDetail-container-image flex justify-between ">
                    <div class="flex flex-start">
                        ...
                        <div class="detail flex flex-column justify-between">
                            ...
                            <div class="flex justify-between align-center">
                                <div class="flex flex-start align-center">
                                    <el-popover placement="top-start" trigger="hover" :content="trialDetail.trialName">
                                        <div slot="reference"
                                            class="detail-trailName view-text marginRight10 font18 fontW7">{{
                                                trialDetail.trialName }}</div>
                                    </el-popover>


                                    <el-popover placement="top-start" trigger="hover" content="修改实验">
                                        <i v-if="userRolesNames != '学生' && !pub"
                                            @click.stop="handleUpdate(trialDetail.trialCourseRelId)" slot="reference"
                            ...
                            <div class="flex flex-start fontW7" style="color: rgb(153, 98, 98);">
                                <icon-text :iconClass="`el-icon-s-custom`" class="marginRight20"
                                    :text="trialDetail.createByName" />
                                <icon-text v-if="userRolesNames != '老师' || pub" :iconClass="`el-icon-s-operation`"
                                    :text="trialDetail.recommendStandard" class="marginRight20" />
                                <icon-text v-else :iconClass="`el-icon-view`"
                                    :text="trialDetail.ofPublic == 1 ? '公开' : '不公开'" class="marginRight20 iconBg" />
                                <icon-text v-if="!pub" :iconClass="`el-icon-s-opportunity`"
                                    :text="trialDetail.courseName || '未关联课程'" />
                            </div>
                            <div class="flex flex-start fontW7" style="color: rgb(153, 98, 98);">
                                <icon-text v-if="userRolesNames != '老师' && !pub" :iconClass="`el-icon-s-check`"
                                    class="marginRight20"
                                    :text="`${trialDetail && trialDetail.userCountInfo && trialDetail.userCountInfo.oneCourseTotal || 0}` + '人'" />
                                <icon-text :iconClass="`el-icon-setting`"
                                    :text="`${trialDetail && trialDetail.userCountInfo && trialDetail.userCountInfo.oneTrialAllCourseFinish || 0}` + '次'"
                                    class="marginRight20" />
                                <icon-text v-if="userRolesNames == '老师' && !pub" :iconClass="`el-icon-s-operation`"
                                    :text="trialDetail.recommendStandard" class="marginRight20" />
                    ...
                    <div class="flex">
                        <div v-if="userRolesNames != '学生' && !pub"
                            ...
                            <div class="val-container"> <span class="val primaryColoro" v-if="trialDetail &&
                                trialDetail.userCountInfo">{{ Number(trialDetail.userCountInfo.oneCourseCorrect) +
        Number(trialDetail.userCountInfo.oneCourseFinish)
    }}</span>
                                <span v-else class="val primaryColoro">0</span>
                                /
                                {{ trialDetail &&
                                    trialDetail.userCountInfo && trialDetail.userCountInfo.oneCourseTotal || 0 }}
                            </div>
                        </div>
                        <div v-if="userRolesNames != '学生' && !pub"
                            class="el-progress-circle-edu marginRight20 flex flex-column  align-center">
                            <el-progress type="dashboard" :stroke-width="20"
                                :percentage="trialDetail.correctPer"></el-progress>
                            <div class="progress-text">实验批改数量</div>
                            <div class="val-container"> <span class="val primaryColoro">{{ trialDetail &&
                                trialDetail.userCountInfo && trialDetail.userCountInfo.oneCourseCorrect || 0 }}</span> /
                                {{ trialDetail &&
                                    trialDetail.userCountInfo && trialDetail.userCountInfo.oneCourseTotal || 0 }}</div>
                        </div>
                        <div :class="userRolesNames != '老师' ? 'justify-center' : 'justify-between'"
                            class=" flex flex-column">
                            <div><el-button v-if="!pub" @click="gotoPage(0)" icon="el-icon-monitor" type="success"
                                    class="editSuccess">去做实验</el-button></div>
                            <div><el-button v-if="userRolesNames == '老师' && !pub" @click="deleteLab" icon="el-icon-delete"
                                    type="danger" class="editDanger" :loading="deleteLoading">删除实验</el-button></div>
                        </div>
                    </div>
                </div>
        ...
        <div>
            <el-tabs class="el-tab-lab" v-model="activeName" @tab-click="handleTabClick">
                <el-tab-pane label="任务列表" name="third">
                    <el-card>
                        <template #header v-if="userRolesNames != '学生' && !pub">
                        ...
                        <task-list ref="task-list" :pub="pub" :trialId="trialDetail.trialId" @editTask="editTask"
                            @openDrawer="openDrawer" @doLab="gotoPage"></task-list>
                    </el-card>
                        ...
                        <el-col :span="12">
                            <div>
                                ...
                                <div class="flex flex-column justify-center el-tab-lab-container padding20">
                                    <video v-if="trialDetail && trialDetail.trialVideoPath" id="video" width="100%"
                                        height="100%" controls>
                ...
                <el-tab-pane v-if="userRolesNames != '学生' && courseId" label="实验监控" name="first">
                    ...
                    <div class="student-container">
                        ...
                        <el-row v-if="newStudentList && newStudentList.length > 0" :gutter="10" class="student-container-el-row">
                            <el-col class="marginBottom20" v-for="(item, index) in newStudentList" :key="index" :xs="4"
                                :sm="4" :md="4" :lg="4" :xl="3">
                                <stu-card @accept="accept" style="margin: 0 auto;" :item="item"></stu-card>
                            </el-col>
                            <el-col></el-col>
                        </el-row>
                        <div v-else style="text-align: center;" class="align-center font18 padding20 color9">暂无数据</div>
                ...
                <el-tab-pane v-if="!pub" label="实验报告" name="second">
                    <detail-table ref="detail-table" @handleCorrention="handleCorrention"
                        @queryTrialReport="queryTrialReport" :trialCourseId="Number(trialId)"></detail-table>
...
export default {
    components: {
        LabList, BorderContainer, EduTinymce,
        IconText, DetailTable, RepDialog, ToolsDetail,
        LabDetail, TaskList, TaskListDialog, TaskContentDrawer, StuCard, StuLabForm
    },
    data() {
        return {
            activeName: 'third',
    ...
    watch: {
        'studentList': function (newVal, oldVal) {
            if (newVal.length > 0) {
                this.newStudentList = this.queryStudentList(newVal, this.stuStatus, this.stuKeyword)
    ...
    methods: {
        ...
        queryStudentList(list, status, keyword) {
            return list && list.filter(item => {
                console.log(item);
                let name = item && `${item.userName}${item.className}${item.studentNum}`
        ...
        handleQueryStu(param) {
            ...
            this.newStudentList = this.queryStudentList(studentList, this.stuStatus, this.stuKeyword)
        ...
        getNewTrialDetail(trialDetail) {
            ...
            let finishPer = Math.floor(((oneCourseCorrect - 0 + oneCourseFinish) / oneCourseTotal) * 100)
        ...
        handleCorrention(obj) {
            ...
            objs = encodeURI(objs);
            this.$router.push({ path: '/lab/correctionExperiment', query: { obj: objs } })
        },
        cancelRepDia() {
        ...
        //删除
        deleteLab() {
            this.$confirm('删除实验将删除实验的全部数据，请谨慎操作！', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                this.deleteLoading = true;
                deleteOneRel({ id: this.trialId }).then(res => {
                    if (res && res.flag) {
        ...
        gotoPage(index = 0, studentId = 0) {
            ...
            let pub = this.pub
            this.$router.push({ path: '/lab/doLab', query: { trialCourseRelId, courseId, reportId, pub, trialItemId, index, studentId } })
        },
        increase() {
            this.percentage += 10;
