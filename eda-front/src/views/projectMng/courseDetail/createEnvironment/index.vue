<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <div v-show="activeValue == 0">
          ...
          <el-form-item label="实例规格">
            <div class="flex" style="width: 600px; padding: 0 20px 0 0px">
              <el-select
                @change="queryStandardsListBy"
                style="width: 130px"
                v-model="queryParams.cpuCoreCount"
                placeholder="请选择"
                id="cpuCoreCount"
              >
            ...
            {{
              (currentRow &&
                ...
                `${currentRow && currentRow.cpuCoreCount}核
                  ...
                  currentRow && currentRow.familyLabel
                }`) ||
              "暂无数据"
            }}
          </el-form-item>
          ...
          <el-form-item label="镜像" prop="region">
            ...
            <div
              class="flex"
              style="width: 600px; padding: 0 20px 0 0px; margin-top: 20px"
            >
              <el-select
                id="CentOS"
          ...
          <el-form-item label="数据盘" prop="diskSizeMB">
            ...
            >
              ...
              <el-input-number
                :disabled="!isCanExpansion"
                slot="reference"
                v-model="ruleForm.diskSizeMB"
                style="width: 120px"
                controls-position="right"
                @change="handleDiskSizeMBChange"
                :min="Number(rebackDiskSizeMB)"
                :max="1024"
                :step="1"
                :step-strictly="true"
              ></el-input-number>
            ...
            &nbsp;GB
            <span style="color: #f00; margin-left: 30px" v-if="!isCanExpansion"
              >*存在设计云快照，如需扩容，请删除设计云快照</span
            >
          </el-form-item>
          ...
          <el-form-item label="快照服务">
            ...
            <el-form-item label="快照保留时间" prop="region">
              保留<el-input-number
                id="region"
                v-model="ruleForm.retentionDays"
                style="width: 120px; margin: 0 10px"
                controls-position="right"
                @change="handleRetentionDaysChange"
                :min="1"
                :max="7"
              ></el-input-number
              >天后自动删除
          ...
          <el-form-item label="设计云人数" prop="maxUserCounts">
            人数上限 &nbsp;<el-input-number
              id="maxUserCounts"
              v-model="ruleForm.maxUserCounts"
              style="width: 120px; margin: 0 10px"
              controls-position="right"
              @change="handleMaxUserCountsChange"
              :min="Number(maxUserCounts)"
              :max="10"
            ></el-input-number
            >&nbsp;<span style="color: #f00; font-size: 12px"
              >注意：该处为每台设计云支持的使用人数范围，最小值{{
                maxUserCounts
              }}，最大值为10</span
            >
          </el-form-item>
          <el-divider></el-divider>
          <el-form-item label="选择IP" prop="eduLibs">
            <el-button size="small" type="primary" @click="handleAdd('ip')"
              >选择IP</el-button
            >
            <div>
              <el-tag
                @close="removeIpList(tag)"
                style="margin-right: 10px"
                v-for="tag in ruleForm.ips"
                :key="tag.id"
                closable
              >
                {{ tag.displayName }}
          ...
          <el-divider></el-divider>
          <el-form-item label="选择PDK" prop="eduLibs">
            <el-button size="small" type="primary" @click="handleAdd('pdk')"
              >选择PDK</el-button
            >
            <div>
              <el-tag
                @close="removePdkList(tag)"
                style="margin-right: 10px"
                v-for="tag in ruleForm.pdks"
                :key="tag.id"
                closable
              >
                {{ tag.displayName }}
          ...
          <div class="text-center">
            ...
            <el-button v-if="!vmId && !vmIds" type="primary" @click="next"
              >下一步</el-button
            >
            <el-button v-else type="primary" :loading="loading" @click="next"
              >修改</el-button
            >
          </div>
        </div>
        ...
        <div v-show="activeValue == 1">
          ...
          <div class="text-center">
            <el-button @click="goToLastPage">取消</el-button>
            <el-button type="primary" @click="activeValue = 0"
              >上一步</el-button
            >
    ...
    <IP-dialog ref="ipDialog" @getIpList="getIpList" />
    <PDK-dialog ref="pdkDialog" @getPdkList="getPdkList" />
    ...
    >
      ...
      >
        <div>
          <i class="el-icon-warning primaryColor"></i
          >当前操作需要实例在关机状态下进行：
        </div>
      ...
      <div>
        ...
        <el-checkbox v-model="updateChecked" id="updateChecked"
          >同意强制关机</el-checkbox
        >
      </div>
...
import IPDialog from "@/views/cloudEnvironment/components/IPDialog.vue";
import PDKDialog from "@/views/cloudEnvironment/components/PDKDialog.vue";
import {
  getVmStandards,
  getVmTypes,
  network,
...
export default {
  ...
  methods: {
    //获取选中的iplist
    getIpList(list) {
      this.ruleForm.ips = list;
    },
    getPdkList(list) {
      this.ruleForm.pdks = list;
    },
    // 关闭关机提示
    dialogCancel() {
    ...
    handleAdd(str) {
      if (str == "ip") {
        let ipDialog = this.$refs["ipDialog"];
        ipDialog.initParams();
        ipDialog.getList();
        ipDialog.dialog.visible = true;
        ipDialog.dialog.status = "create";
        return;
      } else {
        let pdkDialog = this.$refs["pdkDialog"];
        pdkDialog.initParams();
        pdkDialog.getList();
        pdkDialog.dialog.visible = true;
        pdkDialog.dialog.status = "create";
      }
    },
    next() {
      //点击xia一步时，要做到几点
