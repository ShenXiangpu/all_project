<template>
  <div style="position: relative">
    <div id="DataMonitor" :style="{ height: height, width: width }"></div>
    <div class="total primaryColorb fontW7">平台总人数：{{ userData.total || 0 }} 人</div>
  </div>
</template>
...
export default {
  ...
  methods: {
    queryCoursePageForIndex() {
      getCoursePageForIndex().then((res) => {
        if (res && res.flag) {
          let resData = res.resData;
          this.userData = resData;
          this.ageList(resData);
        }
      });
    },
    //处理data
    ...
    ageList(userData) {
      ...
      let _this = this;
      _this.chart = echarts.init(document.getElementById("DataMonitor"));
      let respData = [
        { value: (userData && Number(userData.online)) || 0, name: "在线人数" },
