<template>
  <uploader ref="uploader" @complete="complete" @file-removed="removeFileDefine" @files-submitted="filesSubmitted"
    :options="options" :autoStart="false" :file-status-text="fileStatusText" @file-added="onFileAdded"
    @file-success="onFileSuccess" @file-error="onFileError" @file-progress="onFileProgress" @file-complete="fileComplete"
    ...
    <uploader-drop>
      ...
      <uploader-btn class="chooseBtn">选择文件</uploader-btn>
      <span>注意：文件大小不能超过5G</span>
      <uploader-btn v-if="isDirectory" :directory="true">选择文件夹</uploader-btn>
    </uploader-drop>
    <!-- uploader-list可自定义样式 -->
        ...
        <div class="file-title">
          ...
          <div class="operate">
            <el-button type="text" @click="operate" :title="collapse ? '折叠' : '展开'">
              <i class="icon" :class="collapse ? 'el-icon-caret-bottom' : 'el-icon-caret-top'
                "></i>
            </el-button>
        ...
        <ul class="file-list" :class="collapse ? 'uploader-list-ul-show' : 'uploader-list-ul-hidden'
          ...
          <li v-for="(file, index) in uploadFileList" :key="file.id">
            <div style="position: relative; " class="remove-container">
              <uploader-file class="uploader-file-define" :class="'file_' + file.id" ref="files" :file="file"
                :list="true">

              </uploader-file>
              <div v-if="file.completed" class="pointer remove-icon"
                style="position: absolute;right: 10px;top: 50%;transform: translate(0,-50%);z-index: 3000;">
                <i class="el-icon-remove-outline font20 pointer" @click="removeFileDefine(file, index)"></i>
              </div>
            </div>
...
export default {
  props: {
    target: {
      default: '/edu/zkxy-vmware-api/file/upload',
      type: String
    },
    isDirectory: {
      default: false,
