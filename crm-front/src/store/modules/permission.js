...
import { listRoutes } from '@/api/crm/user';
...
export function filterAsyncRoutes(routes) {
  ...
  routes.forEach((route) => {
    ...
    // let userRolesNames = store.state.user.userRolesNames
    // if (userRolesNames === '老师' && tmp.path === "usersMng") {
    //   tmp.meta.title = '学生管理'
    // }
    ...
    if (component == 'Layout') {
      ...
      const components = import.meta.glob('@/views/**/*.vue') // 会匹配 src/views 目录下所有.vue文件
      tmp.component = components[`/src/views/${component}.vue`]
