<template>
    <div class="app-container fontW7">
        <el-card>
            <template #header>
                <el-upload class="upload-demo" ref="upload" action :http-request="doUpload" :show-file-list="false"
                    :auto-upload="true">
        ...
        <!-- <el-card class="marginTop10">
            ...
            <el-row class="flex  align-center">
                <el-col :xs="24" :sm="24" :md="24" :lg="4" :xl="4">
                    <el-tabs class="el-tabs-edu" tab-position="left" style="height: 200px;" @tab-click="handeClickName">
                        <el-tab-pane  class="el-tabs-pane-edu" v-for="(item, index) in hostSystemList" :key="index" :label="item"></el-tab-pane>
                    </el-tabs>
                </el-col>
                    ...
                    <el-card style="width:80%" v-loading="cardloading">
                        <div class="flex flex-column justify-between">
                            <div class="flex">
                                ...
                                <div>{{ hostSystemInfo && hostSystemInfo.model }}</div>
                            </div>
                            <div class="flex">
                                <div class="font14">处理器类型：</div>
                                <div>{{ hostSystemInfo && hostSystemInfo.cpuModel }}</div>
                            </div>
                            <div class="flex">
                                <div class="font14">逻辑处理器：</div>
                                <div>{{ hostSystemInfo && hostSystemInfo.numCpuThreads }}</div>
                            </div>
                            <div class="flex">
                                <div class="font14">网卡：</div>
                                <div>{{ hostSystemInfo && hostSystemInfo.numNics }}</div>
                            </div>
                            <div class="flex">
                                <div class="font14">实操云：</div>
                                <div>{{ hostSystemInfo && hostSystemInfo.vms }}</div>
                            </div>
                            <div class="flex">
        ...
        <el-dialog :close-on-click-modal="false" :title="textMap[dialog.status]" :visible.sync="dialog.visible" @close="cancel" class="dialog"
            width="50vw">
...
export default {
    ...
    methods: {
        ...
        async handleDelete(row) {
            const { vmId } = { ...row }
            this.$confirm('删除实操云会同步删除该实操云中的所有数据，无法恢复，请慎重选择！', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
