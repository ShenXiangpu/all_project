...
import { getSupplierList } from "@/api/crm/supplier";
...
export default {
  ...
  methods: {
    handleCustomerInfo(row) {
      ...
      const customInfoDialog = this.$refs["custom-info-dialog"];

      // 安全地处理location字段，不影响原始数据
      let location = clonedRow && clonedRow.location;

      // 只在副本上操作，避免修改原始列表数据
      if (location && typeof location === 'string' && location.includes(",")) {
        clonedRow.location = location.split(",");
      }

      // 使用处理后的副本填充表单
      let form = { ...clonedRow };
      customInfoDialog.form = form;

      // 安全地设置ID值
      if (clonedRow.clueId) {
        customInfoDialog.clueId = clonedRow.clueId;
      }

      if (clonedRow.id) {
        customInfoDialog.customerId = clonedRow.id;
      }

      this.componentName = "CusTable";
      customInfoDialog.dialog.visible = true;
      customInfoDialog.dialog.status = "look";
    },
    ...
    getList() {
      this.loading = true;
      getSupplierList(this.listQuery).then((reponse) => {
