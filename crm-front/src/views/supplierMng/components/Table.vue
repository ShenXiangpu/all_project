  ...
  <el-card>
    <template #header v-if="!isDetail">
      <div class="flex justify-between">
        <div>
          <el-button v-permission="['supplier:supplierInfo:addOne']" type="primary" icon="el-icon-plus"
            @click="openFileDialog">添加供应商</el-button>
        </div>

        <div v-if="
          checkPermission(['supplier:supplierInfo:addOne']) ||
          checkPermission([`supplier:supplierInfo:customerList`])
        ">
    ...
    <el-table border :data="tableData" style="width: 100%" v-loading="loading" @selection-change="handleSelectionChange"
      ...
      <el-table-column prop="customerName" label="供应商名称" min-width="100" align="center" show-overflow-tooltip>
      ...
      <el-table-column prop="supplierTypeName" label="供应商类别" align="center" min-width="80" show-overflow-tooltip>
      </el-table-column>
      <el-table-column prop="cooperationTypeName" label="合作类型" align="center" min-width="80" show-overflow-tooltip>
      </el-table-column>
      ...
      <el-table-column prop="consumerSourceName" label="客户来源" align="center" min-width="100" show-overflow-tooltip>
      </el-table-column>
      ...
      <el-table-column label="操作" fixed="right" align="center" min-width="180" v-if="
        !isDetail &&
        (checkPermission(['supplier:supplierInfo:customerList']) ||
          checkPermission([`supplier:supplierInfo:editOne`])) || checkPermission([`supplierFollowUp:addOne`])
      ">
        <template #default="scope">
          <div>
            <el-button v-permission="['supplierFollowUp:addOne']" type="primary" size="mini"
              @click="handleFollow(scope.row)">跟进</el-button>
            <el-button v-permission="['supplier:supplierInfo:editOne']" type="danger" class="editDanger" size="mini"
              @click="handleUpdate(scope.row)">编辑</el-button>
            <el-popconfirm title="确定删除吗？" @onConfirm="handleDelete(scope.row)" onCancel="">
              <el-button v-permission="['supplier:supplierInfo:customerList']" size="mini" type="warning"
