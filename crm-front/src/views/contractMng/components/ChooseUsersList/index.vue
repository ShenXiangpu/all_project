<template>
  <div class="choose-container">
    <el-button type="primary" size="small" @click="addPromoter"
      >添加用户</el-button
    >
    ...
    <span
      @click="removeAll"
      v-show="flowPermission && flowPermission.length && status != 'look'"
      class="marginLeft10 font16 pointer primaryColorb"
      >全部移除</span
    >
    <employees-dialog
...
export default {
  ...
  methods: {
    ...
    removeName(tag) {
      ...
      this.list =
        this.flowPermission &&
        this.flowPermission.length > 0 &&
        this.flowPermission.map((item) => {
          return { userId: item.targetId };
        });
...
.choose-container {
  border: 1px solid #fff; //在表单中 补偿1px 导致el-col样式问题
}
</style>
