<template>
  <div class="container">
    <el-form
      ...
      class="demo-ruleForm"
    >
      <approval-info :ruleForm="ruleForm" />
    </el-form>
...
import { add, edit, getDetailById, getWorkFlowData } from "@/api/crm/approval";
...
export default {
  ...
  created() {
    const { id, type = this.op } = this.$route.query;
    if (type == "create") {
  ...
  methods: {
    async queryDetailById(id) {
      ...
      if (res && res.resData) {
        ...
        const { procName, procType, remark, nodeConfig, status, id } = resData;
        this.ruleForm = {
          procName,
          procType,
          remark,
        ...
        this.id = id;
        this.flowPermission = nodeConfig && nodeConfig.nodeUserList;
        this.nodeConfig = nodeConfig;
        this.nodeConfig.nodeUserList = [];
        this.directorMaxLevel = 4;
        this.processConfig.nodeConfig = nodeConfig;
        this.processConfig.flowPermission =
          nodeConfig && nodeConfig.nodeUserList;
      }
    },
    ...mapMutations("approval", ["setTableId", "setIsTried"]),
    toReturn() {
    ...
    cancelSet() {
      this.$router.push({
        path: "/approvalMng/approvalRules",
      });
    },
    saveSet() {
      this.$refs["ruleForm"].validate(async (valid) => {
        if (valid) {
          let processConfig = {};
          const { procName, procType, remark, status } = this.ruleForm;
          processConfig.procName = procName;
          processConfig.procType = procType;
          processConfig.remark = remark;
          processConfig.status = status;
          ...
          const nodeConfig = this.processConfig.nodeConfig;
          console.log(nodeConfig);

          const flowPermission = this.flowPermission;
          if (flowPermission && flowPermission.length > 0) {
            nodeConfig.nodeUserList = flowPermission;
            nodeConfig.settype = "1";
          } else {
            nodeConfig.settype = "0";
          }
          processConfig.nodeConfig = nodeConfig;
          let res = null;
          if (this.op == "update") {
            processConfig.id = this.id;
            res = await edit(processConfig);
          } else {
            res = await add(processConfig);
          }
