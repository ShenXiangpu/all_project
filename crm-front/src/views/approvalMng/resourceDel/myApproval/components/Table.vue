    ...
    <el-table border :data="tableData" style="width: 100%" v-loading="loading">
      ...
      >
        <template #default="scope">
          <div class="primaryColorb pointer" @click="showApproInfo(scope.row)">
            {{ scope.row.instType }}
          </div>
        </template>
      </el-table-column>
      ...
      >
        <template #default="scope">
          <div>
            <el-button
              v-if="scope.row.approvalStatus == '已撤销'"
              type="primary"
              class="editPrimary"
              size="mini"
            ...
            <el-button
              ...
              :disabled="scope.row.approvalStatus == '已结束'"
              >撤销</el-button
            >
            <el-button
              type="danger"
              ...
              :disabled="scope.row.approvalStatus != '已撤销'"
              >修改</el-button
            >
            <el-popconfirm
              title="确定删除吗？"
...
export default {
  ...
  methods: {
     ...
     */
    showApproInfo(row) {
      this.$emit("showApproInfo", row,"look");
    },
    ...
    handleUpdate(item) {
      this.$emit("showApproInfo", item,'update');
    },
    handleSubmit(row) {
      this.handleMessage(
        "提交后，将进行正常的审批流程，确认填写无误后点击【确认】即可",
        "再次提交",
        "handleSubmit",
        row
      );
    ...
    handleMessage(info, title, op, param) {
