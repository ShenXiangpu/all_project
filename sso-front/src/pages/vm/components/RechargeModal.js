...
class RechargeModal extends PureComponent {
  ...
  componentDidMount() {
    const { vmInfo, rechargeList } = this.props;
    if (vmInfo && vmInfo.vmId && rechargeList && rechargeList.length > 0) {
  ...
  componentDidUpdate(preProps) {
    const { rechargeList, visible } = this.props;
    const { rechargeList: old_rechargeList, visible: old_visible } = preProps;
  ...
  // 计算续费VM相关计费
  calcVmCost = (value) => {
    const { onCalcCost, rechargeList, vmInfo } = this.props;
  ...
  getNewExpireTime = () => {
    const { vmInfo, rechargeList } = this.props;
    const { checkedTimeId } = this.state;

    const vmExpireTime = vmInfo && vmInfo.expirationTime;
  ...
  render() {
    const { form, loading, vmInfo, calcVmPrice, calcLoading, rechargeList, ...modalProps } = this.props
    ...
    return (
      ...
      >
        <Spin spinning={loading}>
          ...
          <span>
            <span>续费后到期时间</span>
            <span style={{ marginLeft: '20px', color: '#000' }}>{this.getNewExpireTime()}</span>
          </span>
          <br />
