...
class FileUpload extends React.Component {
  ...
  createWebUploaderInterface() {
    ...
    console.log(formData, accept, '------------------------------------------uploadUrl',uploadUrl);
  ...
  bindWebUploaderEvent() {
    this.uploader
      ...
      .on('uploadStart', this.handleUploadStart)            // 某个文件开始上传前触发，一个文件只会触发一次。
  ...
  handleBeforeFileQueued = file => {
    ...
    const { onShowModal, beforeFileQueued, onSetFileList, fileList } = this.props;

    // const flag = beforeFileQueued(file);
    // if (flag && isEqual(flag, true)) {
    // 弹出框展示上传列表
    console.log('this.uploader=======================>', this.uploader);
    onShowModal(this.uploader);
    // }

    const fileArr = Array.from(new Set([...fileList, file]));
    onSetFileList(fileArr)
  }
  ...
  handleUploadStart = file => {
    console.log('handleUploadStart >>>>>>', file, this.uploader);
    const { dispatch, fileList, onSetFileList, rootPath, projectId } = this.props;
    ...
    const fileSize = file.size;    //单位：b
    const fileSizeKb = (fileSize / 1024).toFixed(2) + 'KB';   //转换为kb
    // new Promise((reslove, reject) => {
    //   dispatch({
    //     type: 'mpw/isEnoughStorage',
    //     payload: {
    //       fileSize: fileSizeKb,
    ...
    //       const resFlag = response.resData;
    //       if (isEqual(resFlag, false)) {
    //         Modal.error({
    //           title: '存储空间已满，请联系管理员扩容',
    //         });
    //         fileList.map(item => {
    //           this.uploader.skipFile(item);
    //         })
    //         onSetFileList([])
    //       }
    //     } else {
  ...
  handleBeforeSend = (block, data, headers) => {
    ...
    const { rootPath, pickerId, isDirectory, currentPathId, projectId } = this.props;
    ...
    data.md5 = fileMd5 || '';    // data 中 加入 md5 值
    data.chunk = chunk;
    data.chunks = chunks;
  ...
  handleUploadAccept = (file, ret) => {
    ...
    console.log('*** handleUploadAccept >>>>>> file:', file, ret);
    // const { refreshList } = this.props;
    if (ret.flag && ret.resData && ret.resData.flag) {
      onSetValue(ret.resData.fileId)
    }
    return ret.flag;
  }
  ...
  handleuploadComplete = file => {
    ...
    const { onSetFilePath } = this.props
    onSetFilePath(file)
    // 删除进度条
  }
...
FileUpload.propTypes = {
  ...
  uploadBigFileUrl: PropTypes.string,         // 后台接口
