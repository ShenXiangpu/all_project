  ...
  render() {
    ...
    const columns = [
          ...
          if (record.dir) {
            return (
              <div className={styles.file}><Icon className={styles.svg} component={FolderSvg} />
                <div className={styles.fileText} title={record.pathName}>{record.fileName}</div>
              </div>
            )
          } else {
            // 判断文件类型，判断是否为压缩包类型
      ...
      },
      {
        ...
        key: 'createTime',
        width: 110,
        render: text => <span style={{ fontSize: 12, color: '#afb3bf' }}>{ moment(text).format('YYYY-MM-DD HH:mm:ss') }</span>
      },
      {
        title: '操作',
        key: 'operation',
