 ...
 */
  ...
  get reviewProps() {
    ...
    return {
      ...
      projectId,
      onWatchDetailById: () => {
        let pathname = `/mpw/mpwDetail?id=${id}&op=3&status=${status}&projectId=${projectId}`

        if(!projectId){
          pathname = `/mpw/mpwDetail?id=${id}&op=3&status=${status}`
        }
  ...
  get finishProps() {
    ...
    return {
      ...
      projectId,
      onWatchDetailById: () => {
        const pathname = `/mpw/mpwDetail?id=${id}&op=3&status=${status}&projectId=${projectId}`
        router.push(
          pathname
        )
