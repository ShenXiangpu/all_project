        ...
        <Row gutter={[24, 16]} style={{ minHeight: '55vh' }}>
          {
            sourcesList && sourcesList.length > 0 ? sourcesList && sourcesList.length > 0 && sourcesList.map(item => {
              return (
                <Col xs={24} sm={24} md={12} lg={8} xl={5} style={{ minWidth: '320px' }}>
                  <CourseItem item={item} onSeachForm={this.onSeachForm} onCallMissLi={this.onCallMissLi} />
                </Col>
              )
            }) :
