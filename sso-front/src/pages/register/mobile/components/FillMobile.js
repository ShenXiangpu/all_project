...
@Form.create()
class FillMobile extends Component {
  componentDidMount() {
        ...
        <Form className={styles.form}>
          <FormItem label='手机号' {...formItemLayout}>
            {getFieldDecorator('phone', {
              ...
              <Input autoComplete="off" placeholder="请输入你的手机号" />
            )}
          </FormItem>
          <Row gutter={24}>
          ...
          <Form.Item {...tailFormItemLayout}>
            {getFieldDecorator('agreement', {
              ...
              rules: [
                {
                  required: true,
                  message: '请确认阅读并已同意注册须知'
                },
              ],
            })(
              <Checkbox.Group>
                <Checkbox value={true}>我已阅读并已同意<a target='_blank' rel='noopener noreferrer' href="/agreement/registeContract" >注册须知</a></Checkbox>
              </Checkbox.Group>
            )}
          </Form.Item>
          ...
          <Row style={{ textAlign: 'center', marginTop: '20px' }}>
            <Link to="/register/email">切换成企业账户注册 ></Link>
          </Row>
        </Form>
      </div>
    )
...
FillMobile.propTypes = {
  form: PropTypes.object,
}

export default FillMobile
