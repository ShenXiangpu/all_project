...
import BasicConfig from '../../../../vm/components/BasicConfig';
import ShutdownTip from '../../../../vm/components/ShutdownTip'
...
class ResetVmModal extends PureComponent {
  ...
  componentDidMount() {
    ...
    if (vmList && vmList.length > 0) {
      this.setState({
        currentItem: vmList[0]
      })
    }
  }

  componentDidUpdate(prevProps) {
    const { vmList, visible } = this.props;
    const { vmList: old_vmList } = prevProps;

    if (visible !== prevProps.visible && !visible) {
      this.setState({ currentStep: 0 })
    }

    if (vmList && vmList.length > 0 && !isEqual(vmList, old_vmList)) {
      this.setState({
        currentItem: vmList[0]
      })
    }
  ...
  get basicConfigProps() {
    const { form, toolList, vmTypes, templateList, onGetVmStandardList, onCalcCost, vmList, networkList } = this.props
  ...
  handleNext = currentStep => {
    const { form, openReconfigOrder, vmList, networkList } = this.props;
  ...
  render() {
    const { form, loading, vmList, calcVmPrice, calcLoading, networkList, ...modalProps } = this.props
    const { currentStep, vmTools, instanceConfig, currentItem } = this.state;

    // 配置调整后，记录其他实例的配置信息
    const newVmList = vmList;
    if (isEqual(currentStep, 1)) {
      newVmList.map(item => {
          ...
          {currentStep === 1 &&
            <div>
              <Table
                ...
                dataSource={newVmList}
                rowKey={record => record.vmId}
              />
              <p className={styles.cost}>
                需支付费用合计
                <span className={styles.money}>{calcVmPrice && calcVmPrice.realPrice}元</span>
                <span className={styles.moneyLine}>{calcVmPrice.originPrice}元</span>
              </p>
