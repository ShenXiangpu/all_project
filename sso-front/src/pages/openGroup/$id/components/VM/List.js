...
class List extends PureComponent {
  ...
  handleRechargeClick = (record, e) => {
    e.preventDefault();
    const { onShowRechargeModal } = this.props;
    onShowRechargeModal(record);
  }

  handleUsersClick = (vmId, e) => {
    e.preventDefault();
    const { onShowUsersModal } = this.props;
    onShowUsersModal(vmId);
  }

  render() {
    const { powerStatusLoading, isOwner, showUserInfo, ...tableProps } = this.props;
      ...
      },
      {
        ...
        render: (text, record) => {
          if (isEqual(Number(record.status), 2)) {
            ...
            return <a target="_blank" href={`/vm/${record.vmId}`}>{text}</a>
          }
        }
      },
      {
        ...
        render: (text, record) => {
          if (isEqual(Number(record.status), 2)) {
            ...
            const ListOptions = (
              <div id={record.vmId}>
                {
                  (!isEqual(Number(record.status), 0) && !isEqual(Number(record.status), 4) && !isEqual(Number(record.status), 6))
                  &&
                  <span>
                ...
                {(isEqual(Number(record.status), 0) || isEqual(Number(record.status), 4) || isEqual(Number(record.status), 6)) &&
                  <span>
                    <a href="#" onClick={e => this.handleRechargeClick(record, e)}>
                      续费
                    </a>
                    <Divider type="vertical" />
                  </span>
