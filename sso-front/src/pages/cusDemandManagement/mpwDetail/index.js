                ...
                if (isEqual(Number(status), 1)) {
                    dispatch({
                        ...
                        payload: {
                            ...
                            isDetail: false,
                            showUpload: false,
                            leftReadOnly: true, //左边只读
                            rightReadOnly: true,//右边只读
                        }
                    })
                } else {
                    dispatch({
   ...
   */
    ...
    onMoveOrCopyClick = (e, treeModalType) => {
        ...
        if (isEqual(treeModalType, 'move')) { // 移动
                    ...
                    return list.map(item => {
                        if (key === item.id) {
                            const newItem = {
                                ...
                                id: item.id,
                                isDir: item.dir,
                                filePath: item.path                     // 需要移动的文件/文件夹的当前地址
                            }
            ...
            console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", currentSelectFolder, arr);
