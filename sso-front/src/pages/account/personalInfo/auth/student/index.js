...
class StudentAuth extends React.PureComponent {
  state = {
    imgUrl: ''
  }
  ...
  componentDidMount() {
    ...
    if (identityResult) {
      ...
      } else if (isEqual(identityResult.role, 'student')) { // 已认证：学生
    ...
    return {
      ...
      onUserAuth: (values) => { // 用户认证信息提交
        ...
        if (identityInfo && identityInfo.isVerified && isEqual(identityInfo.isVerified, 3)) {
          // 审核不通过，再次提交信息
          dispatch({
            type: 'userAuth/updateStudentAuth',
            payload: {
              ...values,
              identityCard: imgUrl
            }
