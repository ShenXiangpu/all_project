...
import { floatSub } from 'utils/utils.js';

const balance = store.get('balance');  // 余额

// 支付方式（余额支付、在线支付）选择页面
// 支持同时选择两种支付方式：余额支付、在线支付
// 如果有余额，但是余额不足以支付订单，两种支付方式可同时选择，在线支付的金额 = 应付金额 - 余额
class PayModal extends PureComponent {
  ...
  handleChange = (value) => {
    ...
    if (isEqual(checked, 'all')) {
      if (isEqual(value, 'balance')) {
        this.setState({
          checked: 'online'
        })
      } else if (isEqual(value, 'online')) {
        if (balance >= amountToPay && amountToPay > 0) {
          this.setState({
            checked: 'balance'
          })
    ...
    if (!isEqual(checked, 'balance')) {
      if (isEqual(step, 0)) {
        ...
        if (isEqual(checked, 'online')) { // 单一在线支付方式
          onOnlinePay(['AL']);
        } else if (isEqual(checked, 'all')) { // 余额 + 在线支付
          const arr = ['AB'];  // 余额支付
          arr.push(mode)
          onOnlinePay(arr);
        }
                ...
                <div
                  className={
                    detail && isEqual(detail.totalPrice, 0) ?
                      ...
                      :
                      (balance && balance > 0 ?
                        (isEqual(checked, 'all') || isEqual(checked, 'balance') ? classNames(styles.item, styles.actived) : classNames(styles.item, styles.unCheck))
                        ...
                        {((detail && isEqual(detail.totalPrice, 0)) || (balance && balance > 0 && !isEqual(checked, 'online'))) &&
                          <div className={styles.extra}>
                            <span className={styles.payCost}>
                              ...
                              <span className={styles.cost}>
                                <span className={styles.num}>{balance >= amountToPay ? amountToPay : balance}</span>
                                <span className={styles.unit}>元</span>
                              </span>
                            </span>
                          </div>
                ...
                <div
                  className={
                    detail && isEqual(detail.totalPrice, 0) ?
                      classNames(styles.item, styles.disabled)
                      :
                      (isEqual(checked, 'all') || isEqual(checked, 'online') || (balance < amountToPay)
                        ...
                        {!isEqual(checked, 'balance') && (detail && !isEqual(detail.totalPrice, 0)) &&
                          <div className={styles.extra}>
                            <span className={styles.payCost}>
                              <span className={styles.weak}>支付：</span>
                              <span className={styles.cost}>
                                <span className={styles.num}>{isEqual(checked, 'all') ? floatSub(amountToPay, balance) : amountToPay}</span>
                                <span className={styles.unit}>元</span>
                              </span>
                            </span>
                          </div>
          ...
          <div>
            <div className={styles.row}>
              ...
              <div className={styles.right}>
                ...
                <span>
                  <span>金额：
                    <span className={styles.cost}>￥{isEqual(checked, 'all') ? floatSub(amountToPay, balance) : amountToPay}</span>
                  </span>
                </span>
              </div>
            </div>
