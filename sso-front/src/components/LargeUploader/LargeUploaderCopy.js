 ...
 * 1）页面接受用户传入文件。
 * 2）页面用户点击“上传”。
 * 3）页面将文件的基本信息发送到服务端，包括文件名称，大小，修改时间 或者 文件md5。
 * 4）服务端接受请求，根据md5生成目录。
...
const uploadStatusConfig = {
    ...
    process: {
        ...
        btn: '上传完成',
        barClassName: 'processBar successBar',
    },
    error: {
        btn: '继续上传',
        clickName: 'retry',
        barClassName: 'processBar errorBar',
    },
    pause: {
        btn: '暂停',
        clickName: 'pause',
...
export default class LargeUploader extends React.Component {
    ...
    initWebUploaderHook() {
        ...
        WebUploader.Uploader.register({
            ...
            beforeSend: function (block) {
                //分片验证是否已传过，用于断点续传
                var dfd = new WebUploader.Deferred();
                const { file: { fileMd5, size, ext }, chunk, end, start } = block;
                ...
                $.ajax({
                    ...
                    if (data.exists) {   //若存在，返回失败给WebUploader，表明该分块不需要上传
                        dfd.reject();    // 分片存在，跳过
                    } else {
    ...
    handleBeforeSend = (block, data) => {
        console.log('handleBeforeSend >>>>>>', block, '---', data);
        const { file: { fileMd5, id }, chunks } = block;
    ...
    renderFileList(fileList) {
        console.log('renderFileList >>>>>>', fileList);
        return fileList.map((item) => {
            ...
            return (
                <div key={id} className={styles.fileItem} >
                    ...
                    {/* <span className={styles.$(`fileStatus ${uploadStatus}Status`)} onClick={this[clickName] ? this[clickName](id) : null}>{btn}</span> */}
                    {/* <span onClick={this[clickName] ? this[clickName](id) : null}>{btn}</span> */}
                    <span className={styles.delete} onClick={() => this.pause(id)}>
                        <Icon type="pause-circle" />
                    </span>
                    <span className={styles.delete} onClick={() => this.resume(id)}>
                        <Icon type="right-circle" />
                    </span>
                    <span className={styles.delete} onClick={() => this.remove(id)}>
    ...
    render() {
        ...
        return (
            <div>
                <div className={styles.largeUploader} style={style}>
