        ...
        <div v-show="activeValue == 0">
          ...
          <el-form-item label="选择lab" prop="eduLibs">
            <el-button  type="primary" @click="handleAdd"
              >选择lab</el-button
            ><span class="primaryColoro"> (文件在/home目录下)</span>
            <div>
              <el-tag
                @close="removeLabList(tag)"
                style="margin-right: 10px"
                v-for="tag in ruleForm.eduLibs"
                :key="tag.id"
                closable
              >
                {{ tag.displayName }}
        ...
        <div v-show="activeValue == 1">
          ...
          <!-- <el-form-item label="实操云人数" prop="maxUserCounts">
                        人数上限 &nbsp;<el-input-number id="maxUserCounts" v-model="ruleForm.maxUserCounts"
                            ...
                            style="color:#f00;font-size:12px">注意：该处为每台实操云支持的使用人数范围，最小值1，最大值为10</span>
                    </el-form-item> -->
          <el-divider></el-divider>
...
export default {
  ...
  methods: {
    ...
    confirmUpdate() {
      ...
      // console.log("updateVMInfo", updateVMInfo);
      this.loading = true;
      this.isShowDialog = false;
    ...
    next() {
      ...
      console.log(choosesEdaToolsList);
          ...
          createVM(createVMInfo)
            .then((res) => {
              if (res && res.resData) {
                ...
                let id = this.groupId;
                this.$router.push({
                  path: "/cloudEnvironment/cloudEnvironment",
                  query: { id },
                });
              } else {
