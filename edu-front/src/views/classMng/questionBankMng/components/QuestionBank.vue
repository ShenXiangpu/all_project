<template>
  <div class="">
    ...
    <div class="flex justify-between marginBottom20">
      <div>
        <el-input  @clear="handleQueryList" clearable placeholder="请输入题库名称" v-model="queryParams.keyWord" class="marginRight20 el-input-edu"
          >
          <el-button slot="append" icon="el-icon-search" @click="handleQueryList"></el-button>
        </el-input>

        <el-select  clearable v-model="queryParams.universityName" @change="handleQueryList" placeholder="请选择高校" class="el-select-edu">
    ...
    <div>
      <el-card>
        <el-table :data="questionBankList" :loading="loading" border style="width: 100%">
          ...
          <el-table-column prop="directionName" label="方向" min-width="100" align="center">
          </el-table-column>
          <el-table-column prop="num" label="题目数量" min-width="80" align="center">
          </el-table-column>
          ...
          <el-table-column prop="createName" label="创建人" min-width="100" align="center" show-overflow-tooltip>
          </el-table-column>
          <el-table-column prop="createdAt" label="创建时间" min-width="120" align="center" show-overflow-tooltip>
          </el-table-column>
          <el-table-column prop="address" label="操作" align="center" min-width="150" fixed="right">
            <template #default="scope">
              <el-button @click="updateQuestionBank(scope.row)" type="warning" class="editWarning"
                size="mini">修改</el-button>
              <el-button type="primary" :loading="exportLoading" class="editPrimary marginRight10"
              ...
              <el-popconfirm title="确定删除吗？" @onConfirm="handleDelete(scope.row)" onCancel="">
                <el-button size="mini" type="danger" :loading="deleteLoading" class="editDanger" slot="reference">
                  删除
                </el-button>
              </el-popconfirm>
            </template>
...
import { getQuBankList, exportQuBank, deleteQuBank } from '@/api/edu/question'
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      deleteQuBank({ id: row.id }).then(res => {
        if (res && res.flag) {
