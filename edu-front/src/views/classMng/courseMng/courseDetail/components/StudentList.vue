      ...
      <template #content>
        <div class="" style="padding: 10px 0 0px 10px;">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            ...
            <el-form-item label="实操云环境登录状态" prop="vmStatus">
              <el-select
    ...
    <el-card>
        ...
        <div class="flex justify-between align-center">
          ...
          <div class="flex">
            <edu-card
              :index="1"
              title="在线人数"
              :count="`${(stuDetail && stuDetail.onlineNum) || 0}人`"
            />
            <edu-card
              :index="2"
              title="课程完成"
              :count="`${(stuDetail && stuDetail.courseNum) || 0}人`"
            />
            <edu-card
              :index="3"
              title="实验完成"
              :count="`${(stuDetail && stuDetail.trialNum) || 0}人`"
            />
            <edu-card
              :index="4"
              title="作业完成"
              :count="`${(stuDetail && stuDetail.homeworkNum) || 0}人`"
            />
          </div>
        </div>
      </template>
      <div>
        <el-table :data="studentList" border style="width: 100%">
          ...
          <el-table-column label="课程进度" min-width="160" align="center">
            <template #default="scope">
              <el-progress
                class="el-progress-edu"
                :percentage="
                  Number(scope && scope.row && scope.row.courseFileSchedule)
                "
                :color="customColors"
              ></el-progress>
            </template>
          </el-table-column>
          <el-table-column label="实验进度" min-width="160" align="center">
            <template #default="scope">
              <el-progress
                class="el-progress-edu"
                :percentage="
                  Number(scope && scope.row && scope.row.courseTrialSchedule)
                "
                :color="customColors"
              ></el-progress>
            </template>
          </el-table-column>
          <el-table-column label="作业进度" min-width="160" align="center">
            <template #default="scope">
              <el-progress
                class="el-progress-edu"
                :percentage="
                  Number(scope && scope.row && scope.row.homeworkSchedule)
                "
                :color="customColors"
              ></el-progress>
            </template>
...
import { getUsersNotInCourse } from "@/api/edu/student";
