<template>
  <div class='tree-container'>
    ...
    <div class="tree-item flex justify-between align-center" @click="openTreeItem(item)">
      <!-- icon -->

      <div @click="openTreeItem(item)">
        <icon-define :item="item" @openTreeItem="openTreeItem(item)"></icon-define>
        <span @click.stop="showDrawer(item)" class="pointer" :class="item.content ? 'show-drawer' : 'disable-color'">{{
          item.sectionName }} </span>
      </div>
      <div>
        <span class="pointer" @click.stop="showDrawer(item)">
          <el-tag class="detail-bg-color content-color" :class="item.content ? 'show-drawer' : 'disable-color'">章节内容</el-tag>
        </span>
        <el-divider direction="vertical"></el-divider>
        <template v-if="!isOpenClass">
          <span class=" marginRight10">
            <el-tag class="detail-bg-color check-detail">{{ item.stuRate }}</el-tag>
          </span>
          <span v-if="!isStudent" class="pointer check-detail" @click.stop="queryDetail(item)">
        ...
        <span class="pointer marginRight10" v-if="item.level == 1 && !isStudent">
          <el-tag @click.stop="addSubsection(item)">添加子章节</el-tag>
        </span>
        <span class="pointer marginRight10" v-if="!isStudent && !isOpenClass">
          <el-tag type="success" @click.stop="uploadFile(item)">
            上传文件
          </el-tag>
        </span>
        <span class="pointer marginRight10" v-if="!isStudent">
          <el-tag type="warning" @click.stop="editSubsection(item)">
            修改
          </el-tag>
        </span>
        <span class="pointer" v-if="!isStudent" @click.stop>
