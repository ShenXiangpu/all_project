<template>
  <div class="app-container">
    <el-steps v-if="!vmId" :space="200" :active="activeValue" simple process-status="finish" finish-status="success"
      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
        <div v-show="activeValue == 0">
          ...
          <div v-if="ruleForm.areaType == '集成电路方向'">
            <!-- <el-form-item label="工具选择" prop="companyName">
                        ...
                        <el-divider></el-divider> -->
            <tools :vmId="vmId" :ruleForm="ruleForm" @handleArchitecture="handleArchitecture"
              @handleCompanyName="handleCompanyName" @checkThisTools="checkThisTools"
              @reBackVmDetail="reBackVmDetail" />
          </div>
          ...
          <el-form-item label="实例规格">
            <div class="flex" style="width: 600px; padding: 0 20px 0 0px">
              <el-select @change="queryStandardsListBy" style="width: 130px" v-model="queryParams.cpuCoreCount"
                placeholder="请选择" id="cpuCoreCount">
                <el-option label="全部CPU" value=""> </el-option>
                <el-option v-for="item in cpuList" :key="item" :label="item + ' 核'" :value="item">
                </el-option>
              </el-select>
              <el-select @change="queryStandardsListBy" v-model="queryParams.memorySize"
                style="width: 130px; margin-left: 20px" placeholder="请选择" id="memorySize">
                <el-option label="全部内存" value=""> </el-option>
          ...
          <el-form-item>
            <el-table class="el-table-container" ref="singleTable" border :data="vmStandardsList"
              @current-change="handleCurrentChange" @row-click="handleSelectionChange" max-height="300">
              <el-table-column ref="chooseCurrent" label="选择" align="center" width="65">
                <template #default="scope">
                  <el-radio :label="scope.row.id" v-model="flavorId" @change.native="getCurrentRow(scope.row)">{{ ""
                    }}</el-radio>
                </template>
              </el-table-column>
              <el-table-column property="familyLabel" label="机型" min-width="120">
              </el-table-column>
              <el-table-column property="instanceStandards" label="实操云规格" min-width="120">
              </el-table-column>
          ...
          <el-form-item label="当前选择实例" prop="region">
            {{
              (currentRow &&
                currentRow.cpuCoreCount &&
                `${currentRow && currentRow.cpuCoreCount}核
            ${currentRow &&
                currentRow.memorySize
                }GB ${currentRow && currentRow.familyLabel
                }`) ||
              "暂无数据"
            }}
          </el-form-item>
          ...
          <el-form-item label="镜像" prop="region">
            ...
            <div class="flex" style="width: 600px; padding: 0 20px 0 0px; margin-top: 20px">
              <el-select id="CentOS" style="width: 300px; margin-right: 20px" v-model="value1" placeholder="请选择">
              ...
              <el-select style="width: 300px; margin-right: 20px" v-model="value2" placeholder="请选择">
                <el-option id="CentOSss" label="64位" value="64位"> </el-option>
              </el-select>
              <el-select id="CentOS34234" style="width: 300px" v-model="value3" placeholder="请选择">
                <el-option label="CentOS 7 64位" value="CentOS 7 64位">
                </el-option>
              </el-select>
            </div>
          </el-form-item>
          ...
          <el-form-item label="数据盘" prop="diskSizeMB">
            <el-popover placement="top-start" title="容量范围" width="200" trigger="hover" content="">
              <div>
                {{ `${rebackDiskSizeMB}GB ~ 1024GB` }}
                <div v-if="vmId" style="color: #f00">
                  *扩容磁盘时，请提前备份重要数据
                </div>
              </div>
              <el-input-number :disabled="!isCanExpansion" slot="reference" v-model="ruleForm.diskSizeMB"
                style="width: 120px" controls-position="right" @change="handleDiskSizeMBChange"
                :min="Number(rebackDiskSizeMB)" :max="1024" :step="1" :step-strictly="true"></el-input-number>
            </el-popover>
            &nbsp;GB
            <span style="color: #f00; margin-left: 30px" v-if="!isCanExpansion">*存在实操云快照，如需扩容，请删除实操云快照</span>
          </el-form-item>
          ...
          <el-form-item label="快照服务">
            <el-form-item label="备份日期" prop="dayOfWeek">
              <el-checkbox-group id="dayOfWeek" v-model="ruleForm.dayOfWeek">
                <el-checkbox v-for="item in weekOptions" :key="item.value" :label="item.value" name="type">{{ item.label
                  }}</el-checkbox>
              </el-checkbox-group>
            </el-form-item>
            ...
            <el-form-item label="备份时间点" prop="hours">
              <el-checkbox-group id="hours" v-model="ruleForm.hours">
                <el-checkbox v-for="item in hourOptions" :key="item.value" :label="item.value" name="type">{{ item.label
                  }}</el-checkbox>
              </el-checkbox-group>
            </el-form-item>
            ...
            <el-form-item label="快照保留时间" prop="region">
              保留<el-input-number id="region" v-model="ruleForm.retentionDays" style="width: 120px; margin: 0 10px"
                controls-position="right" @change="handleRetentionDaysChange" :min="1" :max="7"></el-input-number>天后自动删除
            </el-form-item>
          </el-form-item>
          <el-divider></el-divider>
          <el-form-item label="实操云人数" prop="maxUserCounts">
            人数上限 &nbsp;<el-input-number id="maxUserCounts" v-model="ruleForm.maxUserCounts"
              ...
              :min="Number(maxUserCounts)" :max="10"></el-input-number>&nbsp;<span
              style="color: #f00; font-size: 12px">注意：该处为每台实操云支持的使用人数范围，最小值{{
                maxUserCounts
              }}，最大值为10</span>
          </el-form-item>
          <el-divider></el-divider>
          <el-form-item label="选择lab" prop="eduLibs">
            <el-button type="primary" @click="handleAdd">选择lab</el-button><span class="primaryColoro">
              (文件在/home目录下)</span>
            <div>
          ...
          <div class="text-center">
            ...
            <el-button v-if="!vmId && !vmIds" type="primary" @click="next">下一步</el-button>
        ...
        <div v-show="activeValue == 1">
          ...
          <el-form-item label="实操云名称" prop="vmName">
            <el-input id="vmNamea" style="width: 300px" v-model="ruleForm.vmName" placeholder="请输入实操云名称"></el-input>
            <div style="color: #999; font-size: 12px">
              2～80个字符，以大小写字母或中文开头，可包含数字、点号（.）、下划线（_）、半角冒号（:）或连字符（-）
            </div>
          </el-form-item>
          <el-divider></el-divider>
          <el-form-item label="描述" prop="annotation">
            <el-input type="textarea" id="annotation" style="width: 300px" v-model="ruleForm.annotation"></el-input>
            <div style="color: #999; font-size: 12px">
              长度为2～255个字符，不能以http://或https://开头
            </div>
          </el-form-item>
          <el-divider></el-divider>
          <el-form-item label="主机名称" prop="hostName">
            <el-input style="width: 300px" id="hostName" v-model="ruleForm.hostName" placeholder="请输入主机名称"></el-input>
            <div style="color: #999; font-size: 12px">
              Linux 等其他操作系统：长度为 1～63
              个字符，允许使用大小写字母、数字或连字符（-）和点号（.），但不能连续使用点号（.），连字符（-）前后不能是点号（.）。不能以数字、点号（.）或连字符（-）开头或结尾。
            </div>
          </el-form-item>
          <el-divider></el-divider>
    ...
    <el-dialog title="关机提示" :visible.sync="isShowDialog" @close="dialogCancel" class="dialog" width="700px">
      <div style="
        ...
        ">
        <div>
          <i class="el-icon-warning primaryColor"></i>当前操作需要实例在关机状态下进行：
        </div>
        <ul style="padding: 0 0 0 20px">
          <li style="padding: 5px 0">
            1、避免数据丢失，实例将关机中断您的业务，请仔细确认。
          </li>
          <li style="padding: 5px 0">
      ...
      <div>
        强制关机<span style="color: #f00; margin-right: 20px">*</span>
        <el-checkbox v-model="updateChecked" id="updateChecked">同意强制关机</el-checkbox>
      </div>
...
import { getOenList } from "@/api/edu/lab";
import {
  getVmStandards,
  getVmTypes,
  network,
...
export default {
  ...
  data() {
    var checkVmName = (rule, value, callback) => {
      validateVmName({ vmName: value }).then((res) => {
        if (res && res.flag && res.resData) {
          ...
          callback(new Error("实操云名称已存在，请重新输入"));
        }
      });
    };
    var validateHostName = (rule, value, callback) => {
      checkHostname({ hostname: value }).then((res) => {
        if (res && res.flag && res.resData) {
          callback(
            new Error(`主机名已存在，请重新输入。（推荐输入：${res.resData}）`)
          );
        } else {
          callback();
        }
  ...
  created() {
    const maxUserCounts = this.$route.query.maxUserCounts;
    const diskSizeMB = this.$route.query.maxDisk;
    const rebackDiskSizeMB = this.$route.query.maxDisk;
    let vmIds = this.$route.query.vmIds;
    //应对刷新后url中的Array变为 String
    vmIds = vmIds && vmIds.toString().split(",");
    if (maxUserCounts && diskSizeMB) {
      //说明是批量修改
      this.maxUserCounts = maxUserCounts;
      this.diskSizeMB = diskSizeMB;
      this.rebackDiskSizeMB = rebackDiskSizeMB;
      this.vmIds = vmIds;
      this.queryIsExistSnapshot();
      this.ruleForm.maxUserCounts = maxUserCounts;
    }
  ...
  methods: {
    getIP(itemList) {
      this.ruleForm.ips = itemList;
    },
    getPDK(itemList) {
      this.ruleForm.pdks = itemList;
    },
    ...
    confirmUpdate() {
      let updateVMInfo = {
        ...
        edaTools: this.ruleForm.tools,
        flavorId: this.flavorId,
        memory: this.memorySizeMB,
        eduLibs: this.ruleForm.eduLibs,
        maxUserCounts: this.ruleForm.maxUserCounts,
        ips: this.ruleForm.ips,
        pdks: this.ruleForm.pdks,
      };
      if (this.vmId) {
        updateVMInfo.vmId = this.vmId;
        this.loading = true;
        this.isShowDialog = false;
        reConfig(updateVMInfo)
          .then((res) => {
            if (res && res.flag) {
        ...
        this.loading = true;
        this.isShowDialog = false;
        reConfigBatch({ updateVMInfo, vmIds: this.vmIds })
          .then((res) => {
            if (res && res.flag) {
              this.$message.success("修改成功");
              this.$router.back();
    ...
    reBackVmDetail(obj, policyObj) {
      ...
      this.ruleForm.tools = this.choosesEdaToolsList;
      this.flavorId = Number(obj.flavorId);
      //回显当前选择实例
      ...
      let row =
        vmStandardsList &&
        vmStandardsList.filter((item) => {
          return item.id == this.flavorId;
        });
      this.currentRow = row[0];

      this.ruleForm.maxUserCounts = obj.maxUserCounts;
      this.maxUserCounts = obj.maxUserCounts;
      ...
      this.ruleForm.ips = obj.ipsInfoList;
      this.ruleForm.pdks = obj.pdksInfoList;

      this.$refs["ip"].itemList = obj.ipsInfoList;
      this.$refs["pdk"].itemList = obj.pdksInfoList;
    ...
    queryIsExistSnapshot() {
      let vmIds = this.vmIds;
      isExistSnapshot({ vmIds }).then((res) => {
        if (res && res.flag) {
          let resData = res.resData;
          this.isCanExpansion = !resData;
        }
      });
    },
    queryshopList() {
      ...
      snapshotList({ vmId }).then((res) => {
        if (res && res.flag) {
          ...
          if (resData && resData.length > 0) {
            this.isCanExpansion = false;
          } else {
            this.isCanExpansion = true;
          }
        }
      });
    },
    ...
    removeLabList(item) {
      let list =
        this.ruleForm &&
        this.ruleForm.eduLibs.filter((i) => {
          return item.id != i.id;
        });
    ...
    next() {
      ...
      console.log("choosesEdaToolsList", choosesEdaToolsList);
