  ...
  >
    <div class="app-container">
      ...
      >
        <div
          slot="reference"
          class="primaryBgColorb title-container text-center view-text font16"
        >
          {{ title }}
        </div>
      </el-popover>
      <div class="tab-container">
        <div
          class="left"
          :class="index == 1 ? 'check' : 'noCheck'"
          @click="check(1)"
        >
          完成学生列表（{{ complete.length }}人）
        </div>
        <div
          class="right"
          :class="index == 2 ? 'check' : 'noCheck'"
          @click="check(2)"
        >
          未完成学生列表（{{ inComplete.length }}人）
        </div>
      </div>
      <div>
        <el-table
      ...
      <div class="footer-container">
        <div class="flex justify-center footer">
          <el-button class="marginRight20" @click="handleClose">
            关 闭
          </el-button>
          <el-button
            type="primary"
            @click="downloadStu"
            :loadding="downLoading"
          >
            下 载 名 单</el-button
          >
        </div>
