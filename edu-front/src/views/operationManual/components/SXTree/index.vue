  ...
  <div class="tree-container">
    ...
    <div class="tree-item flex justify-between align-center">
      ...
      <div>
        <icon-define
          :item="item"
          @openTreeItem="openTreeItem(item)"
        ></icon-define>
        <span @click="openTreeItem(item)" class="pointer"
          >{{ item.name }}
        </span>
      </div>
      <div v-if="userRolesNames === '系统最高管理员'">
        <!-- <span
          v-if="!isStudent"
          class="pointer check-detail"
          @click="addDescribe(item, false)"
        >
        ...
        <span
          class="pointer marginRight10"
          v-if="(item.level == 1 || item.level == 2) && !isStudent"
        >
          <el-tag @click="addSubsection(item)">添加子模块</el-tag>
        </span>

        <span
          class="pointer marginRight10"
          v-if="(item.level == 1 || item.level == 2 || item.level == 3) && !isStudent"
        >
          <el-tag @click="addDescribe(item, true)">添加描述</el-tag>
        </span>

        <span class="pointer marginRight10" v-if="!isStudent">
          <el-tag type="warning" @click="editSubsection(item)"> 修改 </el-tag>
        </span>
        <span class="pointer" v-if="!isStudent">
          <el-popconfirm
            title="确定删除吗？"
            @onConfirm="delChapterOrFile(item)"
          >
    ...
    <div
      v-if="
        item && item.isOpen && item && item.children && item.children.length > 0
      "
...
import { updateFileLearnRate } from "@/api/edu/courseRourse";
const fileSvg = {
  image: "jpg",
  video: "video",
  text: "txt",
...
export default {
  ...
  methods: {
    ...
    watchFiles(item) {
      ...
      let type = this.fileCategory(fileType);
