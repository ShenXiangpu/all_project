<template>
  <el-container class="el-container-contianer">
    <el-header
      class="el-header-top flex justify-between align-center primayBgColor"
      style=""
    >
      <div class="primaryColory flex align-center">
        <span class="marginRight10" v-if="isAssistance"
          >{{ teaReply ? teaReplyMap[teaReply] : "等待老师协助..." }}({{
            countStr
          }})</span
        >
        <el-button
          type="danger"
          v-if="isAssistance && teaReply != 'accept'"
          size="mini"
          class="editPrimary"
      ...
      <el-button
        ...
        size="mini"
        class="editDanger"
        @click="gotoPage('/lab/labDetail', trialCourseRelId)"
        >退出</el-button
      >
    </el-header>
    <el-container class="flex">
      <div
        style="border-bottom: none; border-top: none; position: relative"
        :style="{ width: leftWidth }"
      >
        <div class="left-container">
          <div class="flex min-width">
            <div
              @click="handleChecked(false)"
              class="pointer"
            ...
            <div
              v-if="userRolesNames == '学生' || (userId && userId != 0)"
              @click="handleChecked(true)"
              class="pointer"
          ...
          <div v-if="!isChecked" class="rep-container" style="">
            ...
            <edu-tinymce
              ...
              class="edu-tinymce"
              :value="trialDetail && trialDetail.manualContent"
              :disabled="true"
            ></edu-tinymce>
            <div class="flex justify-between align-center rep-next-title">
              ...
              <div class="next">
                <el-button
                  :disabled="
                    !(trialItemList && trialItemList.length > 0) ||
                    listIndex == trialItemList.length - 1
                  "
                  type="primary"
                  size="small"
                  @click="handleLastOrNext('next')"
                  >下一个
          ...
          <div
            @click="openAndClose"
            :class="isOpen ? 'aside-icon' : 'aside-icon-close '"
          ></div>
          <div
            @mouseover="handleLeftMove()"
            id="left"
            class="left-container-right"
          ></div>
        </div>
      </div>
      <div
        id="right-container"
        :style="{ position: 'relative', width: `${rightWidth}` }"
      >
        <iframe
          v-if="localUrl"
          :src="localUrl"
        ...
        <div v-if="!localUrl">
          <div
            id="info"
            class="info flex flex-column align-center justify-between"
            :key="item.id"
            v-for="item in vmEnvDetail.userCountsToPwd"
          >
            <div style="width: 90%" class="font16">
              登陆账号：{{ item.accountName }}
            </div>
            <div style="width: 90%" class="font16">
              登陆密码：
              <password :password="item.accountPwd"> </password>
            </div>
            <div style="width: 100%">
              <el-button
    ...
    <div class="tips-container" v-if="localUrl">
      ...
      <el-popover placement="right" trigger="hover">
        <div class="app-container">
          <div v-for="item in vmEnvDetail.userCountsToPwd" :key="item.id">
            <copy
              title="SSH"
              :content="`ssh ${item.accountName}@${vmEnvDetail.ip}-p 22`"
            ></copy>
          </div>
      ...
      <tips
        class="tips-help"
        v-if="userRolesNames == '学生' && !isAssistance"
        @click="remoteAssistance"
        icon-class="el-icon-monitor"
        title="远程协助"
      ></tips>

      <tips
        v-if="userRolesNames == '学生' && isAssistance && teaReply != 'accept'"
        class="tips-help"
        icon-class="el-icon-monitor"
        title="等待协助"
      ></tips>
...
import { getVmByCourseId, getConsoleForUser } from "@/api/edu/cloud";
...
export default {
  ...
  created() {
    ...
    console.log(this.userId, this.userRolesNames);
  },
  mounted() {
    this.$store.dispatch("ws/initAssist", {}); //初始化reply状态
  ...
  methods: {
    //每5s保存一次,点击实验报告触发，点击实验手册取消，退出取消，destory取消，点击提交报告取消
    autoSaveReport() {
      ...
      saveReportI({ report, id: this.reportId })
        .then((res) => {
          if (res && res.flag) {
            let nowDate = new Date().getTime();
            nowDate = this.$moment(nowDate).format("YYYY-MM-DD HH:mm:ss");
            this.$message.success("实验报告在 " + nowDate + " 自动保存成功");
    ...
    async queryEnvDetail() {
      ...
      if (vmEnvDetail) {
        let userCountsToPwd =
          vmEnvDetail &&
          vmEnvDetail.userCountsToPwd &&
          Base64.decode(vmEnvDetail.userCountsToPwd);
        userCountsToPwd = JSON.parse(userCountsToPwd);
        userCountsToPwd &&
          userCountsToPwd.map((i) => {
            for (var key in i) {
              console.log(key);
        ...
        vmEnvDetail.password =
          vmEnvDetail &&
          vmEnvDetail.password &&
          Base64.decode(vmEnvDetail.password);
    ...
    submitReport() {
      ...
      this.$confirm(
        "请确定实验报告是否完成，提交后不能重复提交，如果未完成可做保存处理，是否提交？",
        "确定提交吗？",
        {
          ...
          saveReportI({ report, id: this.reportId })
            .then((res) => {
              if (res && res.flag) {
                ...
                submitReportI({ id: Number(this.reportId) })
                  .then((res) => {
                    if (res && res.flag) {
        ...
        .catch(() => {
          saveReportI({ report, id: this.reportId }).then((res) => {
            if (res && res.flag) {
              this.$message({
                type: "success",
                message: "保存成功",
    ...
    handleChecked(isChecked) {
      ...
      getTrialReportById({ id: this.reportId }).then((res) => {
        if (res && res.flag) {
          this.reportDetail = res.resData;
          this.$refs["edu-tinymce"].content = this.reportDetail.report;
        }
      });
      if (isChecked) {
