<template>
  <el-container class="el-container-contianer">
    <el-header class="el-header-top flex justify-between align-center primayBgColor">
      <div class="primaryColory flex align-center">
      </div>
      <el-button type="danger" size="mini" class="editDanger"
        @click="gotoPage('/lab/labDetail', trialCourseRelId)">退出</el-button>
    </el-header>
    <el-container class="flex">
      <div style="padding:0;border-bottom: none; border-top: none; position: relative" :style="{ width: leftWidth }">
        <div class="left-container">
          <div class="flex min-width">
            <div class="pointer lab-book text-center" style="width: 100%;">
              实验手册
            </div>

          </div>
          <div class="rep-container" style="">
            <!-- <div v-html="trialDetail && trialDetail.manualContent"></div> -->
            <edu-tinymce id="TaskContentDrawer" class="edu-tinymce" :value="trialItem && trialItem.manualContent"
              :disabled="true"></edu-tinymce>
            <div class="flex justify-between align-center rep-next-title">
              <div class="pre">
                <el-button :disabled="listIndex == 0" type="primary" size="small" @click="handleLastOrNext('last')">
                  <i class="el-icon-caret-left"></i>
                  上一个</el-button>
              </div>
              <div class="title">
                <el-tooltip class="item" effect="dark" :content="(trialItem && trialItem.manualTitle) || `-`">
                  <div class="title-width view-text font18 fontW7">
                    {{ Number(listIndex) + 1 }} -
                    {{ (trialItem && trialItem.manualTitle) || "暂无任务" }}
                  </div>
                </el-tooltip>
              </div>
              <div class="next">
                <el-button :disabled="!(trialItemList && trialItemList.length > 0) ||
                  listIndex == trialItemList.length - 1
                  " type="primary" size="small" @click="handleLastOrNext('next')">下一个
                  <i class="el-icon-caret-right"></i>
                </el-button>
              </div>
            </div>
          </div>

          <div @click="openAndClose" :class="isOpen ? 'aside-icon' : 'aside-icon-close '"></div>
          <div @mouseover="handleLeftMove()" id="left" class="left-container-right"></div>
        </div>
      </div>
      <div id="right-container" :style="{ position: 'relative', width: `${rightWidth}` }">
        <iframe v-if="localUrl" :src="localUrl" frameborder="0" class="iframe" allowfullscreen="true" style=""></iframe>
        <div v-if="!localUrl">


          <div id="info" class="info flex flex-column align-center justify-between">

            <div style="width: 90%" class="font16">
              登陆密码：
              <password :password="`zkxy@123`"> </password>
            </div>

            <div style="width: 100%">
              <el-button icon="el-icon-success" style="width: 100%" :loading="vmLoading" class="editSuccess"
                type="success" @click="goToNovnc()">{{ !vmLoading ? `进入实操云环境` : `实验环境配置中，请稍后...` }}</el-button>
            </div>
          </div>
        </div>
      </div>
    </el-container>
    <div class="tips-container" v-if="localUrl">
      <!-- <tips class="tips-download" @click="downloadDialog"></tips> -->

      <!-- <tips class="tips-upload" @click="uploadDialog" icon-class="el-icon-upload" title="上传"></tips> -->
      <el-popover placement="right" trigger="hover">
        <div class="app-container">
          <copy title="密码" :content="`zkxy@123`"></copy>
        </div>
        <div class="login-tips primaryBgColor pointer" slot="reference">
          登录信息
        </div>
      </el-popover>



      <!-- DownloadDialog -->
      <!-- <download-dialog ref="tips-download" :vmId="vmEnvDetail.vmId"></download-dialog> -->
      <!-- <upload-dialog :vmId="vmEnvDetail.vmId" ref="tips-upload"></upload-dialog> -->
    </div>
  </el-container>
</template>

<script>
import {
  trialContainerStatus,
  oneTrialTempInfo,
  oneTrialHasItemList,
  taskLogin,
  createContainerByExp
} from "@/api/edu/labCenter";

import {
  sendHelp,
  cancelHelp,
  exitTrial,
  exitTrialTeacher,
} from "@/api/edu/assistance";

import { mapGetters, mapState } from "vuex";
import Password from "./components/Password";
import RepDialog from "../components/RepDialog.vue";
import { Base64 } from "js-base64";
import EduTinymce from "@/components/Edu-tinymce";
import Tips from "./components/Tips";
import Copy from "./components/Copy";
import DownloadDialog from "./components/DownloadDialog";
import UploadDialog from "./components/UploadDialog";
import XtermContainer from "./components/XtermContainer.vue";
import { useWatermark } from "@/utils/watermark";
import { waterMarkerInfo } from "@/settings";
import { set } from "nprogress";
const teaReplyMap = {
  accept: "老师正在协助...",
  reject: "老师已拒绝协助",
  completed: "",
};

export default {
  components: {
    EduTinymce,
    Password,
    RepDialog,
    Tips,
    Copy,
    DownloadDialog,
    UploadDialog,
    XtermContainer,
  },
  data() {
    return {
      teaReplyMap: teaReplyMap,
      leftWidth: "700px",
      leftWidthNumber: 700,
      rightWidth: "calc(100vw - 700px)",
      trialCourseRelId: 0,
      courseId: 0,
      isOpen: true,
      isChecked: false,
      trialDetail: {},
      submitLoading: false,
      vmEnvDetail: {},
      localUrl: "",
      reportDetail: {},
      dialogTableVisible: false,
      pub: false,
      listIndex: 0,
      trialItemId: 0,
      trialItemList: [],
      gridData: [],
      filesList: [],
      taskID: 0,
      isAssistance: false, //是否协助
      count: 0,
      countTimer: null,
      userId: "",
      countStr: "0秒",
      saveLoading: false,
      autoSaveTimer: null,
      mask: null,
      vmLoading: false,
      trialItem: {},
      localWaitUrl: "",
      statusClear: null,
    };
  },
  computed: {
    ...mapGetters(["userRolesNames", "teaReply"]),
    ...mapState({
      name: state => state.user.name
    })
  },
  watch: {
    isChecked: {
      handler(newVal) {
        if (!newVal) {
          setTimeout(() => {
            const tinymce = document.getElementById("TaskContentDrawer");
            if (tinymce) {
              const { setWatermark } = useWatermark(tinymce);
              setWatermark(`${this.name}`);
            }
          }, 500);
        } else {
          const tinymce = document.getElementById("TaskContentDrawer");
          if (tinymce) {
            const { clear } = useWatermark(tinymce);
            clear();
          }
        }
      },
      immediate: true,
    },
    isOpen: {
      handler(newVal) {
        if (newVal) {
          setTimeout(() => {
            const tinymce = document.getElementById("TaskContentDrawer");
            if (tinymce) {
              const { setWatermark } = useWatermark(tinymce);
              setWatermark(`${this.name}`);
            }
          }, 500);
        } else {
          const tinymce = document.getElementById("TaskContentDrawer");
          if (tinymce) {
            const { clear } = useWatermark(tinymce);
            clear();
          }
        }
      },
      immediate: true,
    },
    leftWidthNumber: {
      handler(newVal) {
        setTimeout(() => {
          const tinymce = document.getElementById("TaskContentDrawer");
          if (tinymce) {
            const { setWatermark, clear } = useWatermark(tinymce);
            clear();
            setWatermark(`${this.name}`);
          }
        }, 500);
      },
      deep: true,
    },
  },
  destroyed() {
    this.isOpen = true;
    this.isChecked = false;
    this.trialDetail = {};
    this.submitLoading = false;
    this.vmEnvDetail = {};
    this.localUrl = "";
    this.localWaitUrl = "";
    clearInterval(this.statusClear);
  },
  created() {
    this.trialCourseRelId = this.$route.query.trialCourseRelId;
    this.trialItemId = this.$route.query.trialItemId;
    this.pub = this.$route.query.pub;
    this.listIndex = this.$route.query.index;
  },
  mounted() {
    if (this.trialCourseRelId) {
      oneTrialTempInfo(this.trialCourseRelId).then((res) => {
        if (res && res.flag) {
          this.trialDetail = res && res.resData
        }
      });
      oneTrialHasItemList({ trialId: this.trialItemId }).then((res) => {
        if (res && res.flag) {
          const resData = res.resData;
          this.trialItemList = resData;
          this.trialItem = resData && resData[this.listIndex];
          this.taskID = this.trialItem && this.trialItem.id;
        }
      });
    }
  },
  methods: {
    async handleVmStatus() {
      let trialDetail = this.trialDetail;
      if (!trialDetail) {
        return;
      }
      let id = trialDetail.trialId;
      let edaTools = trialDetail.tools;
      let eduLibs = trialDetail.labList || [];
      let ipsInfoList = trialDetail.ipList || [];
      let pdksInfoList = trialDetail.pdkList || [];

      let item = {
        id,
        edaTools,
        eduLibs,
        ipsInfoList,
        pdksInfoList,
      };

      const res = await trialContainerStatus(item);
      if (res && res.flag) {
        let resData = res.resData;
        let ofReady = resData.ofReady;
        if (ofReady == 1) {
          clearInterval(this.statusClear);
          this.localUrl = this.localWaitUrl;
          this.vmLoading = false;
        }
      } else {
        clearInterval(this.statusClear);
        this.vmLoading = false;
      }
    },
    //打开实验环境
    goToLab(trialDetail) {
      let id = trialDetail.trialId;
      let edaTools = trialDetail.tools;
      let eduLibs = trialDetail.labList || [];
      let ipsInfoList = trialDetail.ipList || [];
      let pdksInfoList = trialDetail.pdkList || [];

      let item = {
        id,
        edaTools,
        eduLibs,
        ipsInfoList,
        pdksInfoList,
      };
      this.vmLoading = true;
      createContainerByExp(item).then((res) => {
        if (res && res.flag) {
          let resData = res.resData;
          let ofInitCreate = resData.ofInitCreate;
          let localUrl = resData.vncUrl;
          //已经创建
          if (ofInitCreate == 2) {
            this.localUrl = localUrl;
          } else if (ofInitCreate == 1) {
            this.localWaitUrl = localUrl;
            if (this.statusClear) {
              clearInterval(this.statusClear);
            }
            this.statusClear = setInterval(() => {
              this.handleVmStatus();
            }, 2000);
          }

        }
      }).catch((err) => {
        this.vmLoading = false;
        this.$message.error("实验环境创建失败，请稍后重试");
      })
    },

    handleLastOrNext(str) {
      let index = this.listIndex;
      let trialItemList = this.trialItemList;
      let listLength = trialItemList && trialItemList.length;

      if (str == "last") {
        index > 0 && index--;
        this.listIndex = index;
      } else {
        index < listLength - 1 && index++;
        this.listIndex = index;
      }
      this.trialItem = this.trialItemList[index];
      this.taskID = this.trialItem && this.trialItem.id;
      if (this.taskID) {
        taskLogin({ taskID: this.taskID });
      }
    },

    openAndClose() {
      this.isOpen = !this.isOpen;
      if (this.isOpen) {
        this.leftWidth = "700px";
        this.rightWidth = "calc(100vw - 700px)";
      } else {
        this.leftWidth = "0";
        this.rightWidth = "calc(100vw - 0px)";
      }
    },
    handleLeftMove() {
      const el = document.querySelector("#left");
      const dragDom = el.parentNode.parentNode; // 窗体中间嵌套了一层：dialog -> div -> dragDom
      const dragDomright = el.parentNode.parentNode.nextSibling;
      // 由于使用的是一个指令绑定到了三个 dragdom 上面，所以做了参数判断。
      // corner: 右下角；right: 右侧；bottom：底部；
      // 参数可根据自身编码习惯修改
      const value = "right";
      if (!dragDom) return; // 错误处理
      let _this = this;
      el.onmousedown = (e) => {
        // 鼠标按下，在原来页面上增加透明遮罩，防止部分元素例如iframe监听不到鼠标事件
        const mask = document.createElement("div");
        mask.setAttribute(
          "style",
          "position:fixed;top:0px;bottom:0px;left:0px;right:0px;background:rgba(0,0,0,0)"
        );
        mask.setAttribute("id", "mask");
        document.body.appendChild(mask);
        // 计算当前元素距离可视区的距离
        const disX = e.clientX - el.offsetLeft;
        const disY = e.clientY - el.offsetTop;
        document.body.onmousemove = function (e) {
          e.preventDefault(); // 移动时禁用默认事件

          // 通过事件委托，计算移动的距离
          const l = e.clientX - disX;
          const h = e.clientY - disY;
          if (e.view.innerWidth - e.clientX < 300) {
            // 右边界控制
            return;
          }
          // 一个方法实现三种方式，所以根据参数来判断 size 方向
          if (value === "right" || value === "corner") {
            dragDom.style.width = `${l}px`;
            _this.leftWidth = `${l}px`;
            _this.leftWidthNumber = l;
            dragDomright.style.width = `calc(100vw - ${l}px)`;
            _this.rightWidth = `calc(100vw - ${l}px)`;
          }
          // 判断弹窗高度，防止用于拖动的点移出可视区
          if (value === "bottom" || value === "corner") {
            dragDom.style.height = `${h > document.body.offsetHeight ? document.body.offsetHeight : h
              }px`;
          }
        };

        document.body.onmouseup = function (e) {
          document.body.removeChild(mask); // 移除mask遮罩
          document.body.onmousemove = null;
          document.body.onmouseup = null;
        };
      };
    },

    //查看实验报告


    goToNovnc() {
      let isLicenseEnabled = this.$store.state.user.isLicenseEnabled;
      if (!isLicenseEnabled) {
        this.$message.warning("抱歉，系统已过期，需联系官方工作人员续费");
        return;
      }
      this.goToLab(this.trialDetail);
    },

    //打开上传dialog
    uploadDialog() {
      let tipsUpload = this.$refs["tips-upload"];
      tipsUpload.dialogVisible = true;
      tipsUpload.userId = this.userId;
      tipsUpload.queryListFile();
    },

    gotoPage(path, id) {
      this.$router.push({ path, query: { id, public: this.pub } });
    },


  },
};
</script>

<style lang="scss" scoped>
#right-container {
  overflow: hidden;
}



.tips-container {
  position: absolute;
  top: 190px;
  right: 0px;
  width: 36px;
  height: 80px;
  overflow: hidden;
  // transform: translateX(50px);

  .tips-download,
  .tips-switch,
  .tips-upload {
    right: -40px;
  }

  .tips-download {
    top: 40px;
  }

  .tips-switch {
    top: 40px;
  }

  .tips-switch:hover,
  .tips-download:hover,
  .tips-upload:hover {
    right: 0px;
  }

  .tips-help {
    margin-top: 100px;
    width: 100px;
    top: 70px;
    right: -65px;
  }

  .tips-help:hover {
    right: 0px;
  }

  .tips-switch {
    ::v-deep {
      .iconClass {
        transform: rotate(90deg);
      }
    }
  }

  .login-tips:hover {
    right: 0px;
  }

  .login-tips {
    color: #fff;
    padding: 10px 7px;
    font-size: 14px;
    writing-mode: vertical-lr;
    position: absolute;
    right: -26px;
    top: 0px;
    z-index: 9990;
    border-radius: 10px 0 0 10px;
  }

  .tips-help-tea {
    margin-top: 100px;
    width: 130px;
    // transform: translateX(100px);
  }
}

::v-deep {
  .el-header-top {
    height: 50px !important;
    line-height: 50px !important;
    padding: 0 20px !important;
    background: #10abb9 !important;
    // text-align: right;
  }
}

// .edu-tinymce {
//     width: 800px !important;

//     ::v-deep .tox-tinymce {
//         width: 800px !important;
//     }

// }

.el-container-contianer {
  height: 100vh;
  box-sizing: border-box;
  overflow: hidden;
  .left-container {
    box-sizing: border-box;
    overflow: auto;
    height: calc(100vh - 54px);

    .min-width {
      min-width: 200px;
    }

    .rep-container {
      margin: 0 auto;
      padding: 20px;
      overflow: auto;
      height: calc(100vh - 110px);

      .edu-tinymce {
        ::v-deep {

          .tox,
          .tox-tinymce {
            min-width: 200px;
            width: 100% !important;
            height: calc(100vh - 205px) !important;
          }
        }
      }

      .rep-next-title {
        border: 1px solid #10abb9;
        border-radius: 10px 10px 0 0;
        padding: 10px;
        overflow: auto;

        .title {
          .title-width {
            max-width: 400px;
          }
        }
      }
    }

    .rep-container-else {
      margin: 0 auto;
      padding: 20px;
      overflow: auto;
      height: calc(100vh - 110px);

      .edu-tinymce {
        ::v-deep {

          .tox,
          .tox-tinymce {
            min-width: 200px;
            width: 100% !important;
            height: calc(100vh - 250px) !important;
          }
        }
      }
    }

    .lab-book,
    .lab-rep {
      font-size: 20px;
      height: 50px;
      line-height: 50px;
      text-align: center;
      color: #333;
      background: #ddd;
      box-sizing: border-box;
    }

    .checked-class {
      color: #409eff;
      background: #fff;
    }
  }

  .iframe {
    width: 100%;
    height: calc(100vh - 50px);
  }

  .left-container:hover .aside-icon {
    display: block;
  }
}

.closeClass {
  width: 30%;
  transition: all 0.5s;
  /* 动画名称 */
  animation-name: close;
}

.bigClass {
  width: 100%;
  // transition: all 0.5s;
  /* 动画名称 */
}

@keyframes open {
  0% {
    width: 0;
  }

  100% {
    width: 30%;
  }
}

@keyframes close {
  0% {
    width: 30%;
  }

  100% {
    width: 0;
  }
}

.closeClass {
  width: 0;
  position: relative;
}

.info {
  // position: absolute;
  // left: 50%;
  // top: 50%;
  // transform: translate(-50%, -50%);
  margin: 0 auto;
  width: 260px;
  height: 120px;
  margin-top: calc(50vh - 120px);
}

.aside-icon {
  background-image: url("../../../../assets/img/lab/right1.png");
}

.aside-icon-close {
  background-image: url("../../../../assets/img/lab/left1.png");
}

.el-header {
  background-color: #b3c0d1;
  color: #333;
  line-height: 60px;
}

.el-aside {
  color: #333;
}

.left-container-right {
  position: absolute;
  z-index: 2;
  height: calc(100vh - 50px);
  width: 4px;
  right: -4px;
  top: 0;
  cursor: col-resize;
  background: #bbb;
}
</style>
