<template>
  <div>
    <el-form-item :label="`${type}选择`" :prop="types">
      <el-button type="primary" icon="el-icon-plus" class="marginBottom20 marginRight20" @click="openDialog">添加{{ type
      }}</el-button><span class="">已添加</span><span class="primaryColor"> {{ itemList && itemList.length || 0
        }}个</span>
      <el-table :data="itemList" style="width: 100%; margin-bottom: 20px" row-key="id" border default-expand-all
        ...
        <el-table-column prop="name" :label="`${type}名称`" min-width="180" align="center" show-overflow-tooltip>
        </el-table-column>
        <el-table-column prop="version" label="版本名称" min-width="180" align="center" show-overflow-tooltip>
        </el-table-column>
        <el-table-column prop="universityName" label="所属高校" min-width="180" align="center" show-overflow-tooltip>
        </el-table-column>
        <el-table-column prop="supplier" label="供应商" min-width="180" align="center" show-overflow-tooltip>
        </el-table-column>
        <el-table-column prop="process" label="工艺制程" min-width="80" align="center">
    ...
    <iP-and-pDK-dialog ref="iP-and-pDK-dialog" :type="type" @handleSelectionChange="handleSelectionChange" />
  </div>
</template>
