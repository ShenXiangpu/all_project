<template>
  <div class="app-container">
    <border-container class="marginBottom10" :height="15" :isShowTitle="false" :isBgShow="false">
      <template #content>
        <div style="padding: 10px 0 0px 10px;">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            <el-form-item prop="keyWord" :label="userRolesNames == '老师' ? '学生名称' : '用户名称'">
              <el-input class="inputClass" v-model="queryParams.keyWord" :placeholder="userRolesNames == '老师' ? '请输入学生名称' : '请输入用户名称'
                " clearable @keyup.enter="handleQueryInitPage" />
            </el-form-item>
            <get-university :queryParams="queryParams" v-if="userRolesNames == '系统最高管理员'"/>
            <el-form-item prop="roleId" label="角色" v-if="userRolesNames != '老师'">
              <el-select class="inputClass" v-model="queryParams.roleId" @change="handleQueryInitPage"
                placeholder="请选择角色">
            ...
            ">
              <el-form-item prop="grade" label="入学年份">
                <el-input class="inputClass" v-model="queryParams.grade" placeholder="请输入入学年份" clearable
                  @keyup.enter="handleQueryInitPage" />
              </el-form-item>
              <el-form-item prop="className" label="班级">
                <el-input class="inputClass" v-model="queryParams.className" placeholder="请输入班级" clearable
                  @keyup.enter="handleQueryInitPage" />
              </el-form-item>
              <el-form-item prop="studentNum" label="学/工号">
                <el-input class="inputClass" v-model="queryParams.studentNum" placeholder="请输入学/工号" clearable
                  @keyup.enter="handleQueryInitPage" />
              </el-form-item>
            </span>
            ...
            <el-form-item>
              <el-button type="success" class="editSuccess" icon="el-icon-search"
                @click="handleQueryInitPage">搜索</el-button>
      ...
      <template #header v-if="userRolesNames != '老师'">
        <div class="flex justify-between">
          <div>
            ...
            <el-popconfirm title="确定删除吗？" @onConfirm="handleDeleteIds(ids)" onCancel="">
              <el-button type="danger" icon="el-icon-delete" slot="reference" :disabled="ids && ids.length === 0">批量删除
              </el-button>
            </el-popconfirm>
          </div>
          <div class="flex">
            ...
            <el-button style="margin-right: 20px" type="info" class="editInfo" size="small" icon="el-icon-download"
              @click="downLoadFile">导出模板</el-button>
            <el-upload ref="upload" action :http-request="doUpload" :show-file-list="false"
              :before-upload="beforeAvatarUpload" :auto-upload="true">
              <el-button :loading="insertLoading" style="font-size: 12px" slot="trigger" type="info" class="editInfo"
                icon="el-icon-upload2">批量导入</el-button>
            </el-upload>
          </div>
        </div>
      </template>

      <el-table ref="dataTableRef" v-loading="loading" :data="usersList" @selection-change="handleSelectionChange"
        @row-click="handleRowClick">
        <el-table-column type="selection" width="55" align="center" />
        <el-table-column :label="userRolesNames == '老师' ? '学生名称' : '用户名称'" prop="userName" min-width="100"
          align="center" />
        <el-table-column v-if="userRolesNames == '学校管理员'" label="学/工号" prop="studentNum" width="150" align="center" />
        <el-table-column v-else-if="userRolesNames == '老师'" label="学号" prop="studentNum" width="150" align="center" />
        <el-table-column label="角色名称" prop="roleName" min-width="150" align="center" />
         <el-table-column v-if="userRolesNames == '系统最高管理员'" label="所属高校" prop="universityName" min-width="120"
          align="center"></el-table-column>
        <el-table-column label="入学年份" prop="grade" min-width="150" align="center">
          <template slot-scope="scope">
            <div v-if="scope.row.grade">{{ scope.row.grade }}</div>
            <div v-else>- - -</div>
          </template>
        </el-table-column>
        <el-table-column label="班级" prop="className" min-width="150" align="center">
          <template slot-scope="scope">
            <div v-if="scope.row.className">{{ scope.row.className }}</div>
            <div v-else>- - -</div>
          </template>
        </el-table-column>
        <el-table-column label="手机号" prop="phone" min-width="150" align="center" />
        <el-table-column label="邮箱" prop="email" min-width="180" align="center" />
        ...
        <el-table-column label="操作" align="center" fixed="right" min-width="180">
          <template #default="scope">
            ...
            <el-divider direction="vertical" v-if="userRolesNames != '老师'"></el-divider>
    ...
    <el-dialog :title="dialog.title" :close-on-click-modal="false" :visible.sync="dialog.visible" @close="cancel"
      class="el-dialog-edu">
      <el-form ref="dataFormRef" :model="formData" :rules="formDataRules" class="el-form-edu">
        <el-form-item label="用户名称" prop="userName1">
          <el-input v-model="formData.userName1" id="userName2" class="el-input-edu" placeholder="请输入用户名称"
            show-word-limit />
        </el-form-item>
        <el-form-item label="密码" prop="password1">
          <el-input v-model="formData.password1" id="passwords" type="password" class="el-input-edu ipt"
            placeholder="请输入密码" auto-complete="new-password" show-word-limit />
        </el-form-item>
        <!-- 权限标识 -->
        <el-form-item prop="roleName" label="所属学校" v-if="userRolesNames == '系统最高管理员'"
          :rules="[{ required: true, message: '请选择学校', trigger: 'blur' }]">
          <el-select class="el-input-edu" v-model="formData.university" id="universityName" placeholder="请选择学校"
            @change="changeSchool">
            <el-option v-for="item in schoolList" :key="item.id" :label="item.universityName"
              :value="item.id"></el-option>
          </el-select>
        </el-form-item>
        <!-- 权限标识 -->
        <el-form-item prop="roleName" label="所属角色">
          <el-select class="el-input-edu" v-model="formData.roleName" id="roleId" placeholder="请选择角色"
            @change="handleRoles">
            <el-option v-for="item in roleList" :key="item.roleId" :label="item.roleName"
              :value="item.roleId"></el-option>
          </el-select>
        ...
        <el-form-item label="手机号" prop="phone">
          <el-input maxlength="11" class="el-input-edu" v-model="formData.phone" placeholder="请输入手机号" />
        </el-form-item>
        <el-form-item label="邮箱" prop="email">
          <el-input class="el-input-edu" v-model="formData.email" placeholder="请输入邮箱" />
        </el-form-item>
        <el-form-item v-if="roleName == '学生'" label="学号" prop="studentNum" :rules="[
          { required: true, message: '请输入学号', trigger: 'blur' },
          { pattern: /^[0-9]*$/, message: '请输入数字学号' },
        ]">
          <el-input v-model="formData.studentNum" class="el-input-edu" placeholder="请输入学号" />
        </el-form-item>
        <el-form-item v-if="roleName == '老师'" label="工号" prop="studentNum" :rules="[
          { required: true, message: '请输入工号', trigger: 'blur' },
          { pattern: /^[0-9]*$/, message: '请输入数字工号' },
        ]">
          <el-input v-model="formData.studentNum" class="el-input-edu" placeholder="请输入工号" />
        </el-form-item>
        <el-form-item v-if="roleName == '学生'" label="入学年份" prop="grade">
          <el-input v-model="formData.grade" class="el-input-edu" placeholder="请输入入学年份" />
        </el-form-item>
        <el-form-item v-if="roleName == '学生'" label="班级" prop="className">
          <el-input v-model="formData.className" class="el-input-edu" placeholder="请输入班级" />
        </el-form-item>
    ...
    <show-user-dialog ref="show-user-dialog" />
  </div>
</template>
<script>
import {
...
import { getSchoolList } from "@/api/edu/school";
...
export default {
  ...
  methods: {
    ...
    changeSchool(val) {
      console.log(val);
      this.formData.universityName = val;
        ...
        .then((res) => {
          ...
          if (res && res.flag) {
            ...
            if (resData && resData.length > 0) {
              const show = this.$refs["show-user-dialog"];
              show.handleOpen();
              show.tableData = resData;
            } else {
    ...
    handleUpdate(item) {
      if (item && item.id) {
        getUserDetailById({ id: item.id }).then((res) => {
          ...
          this.userId = item.id;
          let formData = {
            userName1: resData.userName, // 父级菜单
            roleId: resData.roleId,
            roleName: resData.roleName,
            password1: resData.password,
            phone: resData.phone,
            university: resData && resData.universityName,
    ...
    async showRoleMenuDialog(e) {
      ...
      if (res && res.flag) {
        const resData = res.resData;
        const roleMenuList = this.handleTreeList(resData); // 获得需要回显的菜单，需要对父级菜单进行过滤
        const defaultCheckedKeys = this.handleRoleMenuList(roleMenuList);
