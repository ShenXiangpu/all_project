    ...
    <el-card>
      <template #header>
        <el-upload
          ...
          :auto-upload="true"
        >
          <el-button slot="trigger" type="primary" icon="el-icon-plus"
            >添加许可证</el-button
          >
        </el-upload>
      </template>
      <el-table :data="courseList" style="width: 100%" v-loading="loading">
        ...
        >
          <template #default="scope">
            <div>
              {{
                $moment(scope.row.licenseDeadline).format("YYYY-MM-DD HH:mm:ss")
              }}
            </div>
          </template>
        </el-table-column>
        <el-table-column prop="" label="状态" min-width="200" align="center">
          <template #default="scope">
            <el-button
              :type="scope.row.index == 0 ? 'primary' : 'warning'"
              size="mini"
              >{{ scope.row.index == 0 ? "生效中" : "已失效" }}</el-button
            >
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-row class="flex align-center">
        <el-col :xs="24" :sm="24" :md="24" :lg="4" :xl="4">
          <el-tabs
            ...
            tab-position="left"
            style="height: 200px"
            @tab-click="handeClickName"
          >
            <el-tab-pane
              v-for="(item, index) in hostSystemList"
              :key="index"
              :label="item"
            ></el-tab-pane>
          </el-tabs>
                ...
                <div
                  :style="{
                    color:
                      ...
                      POWERSTATE[hostSystemInfo.connectionState].color,
                  }"
                >
                  {{
                    POWERSTATE[hostSystemInfo.connectionState] &&
                    POWERSTATE[hostSystemInfo.connectionState].text
                  }}
                </div>
              </div>
              <div class="flex">
                ...
                <div>
                  <el-progress
                    ...
                    :percentage="
                      Number(
                        (
                          (Number(hostSystemInfo.cpuUsage) /
                            Number(hostSystemInfo.cpuCapacity)) *
                          100
                        ).toFixed(0)
                      ) || 0
                    "
                ...
                <div>
                  <el-progress
                    ...
                    :percentage="
                      Number(
                        (
                          (Number(hostSystemInfo.memoryUsage) /
                            Number(hostSystemInfo.memoryCapacity)) *
                          100
                        ).toFixed(0)
                      ) || 0
                    "
                ...
                <div>
                  <el-progress
                    ...
                    :percentage="
                      Number(
                        (
                          (Number(hostSystemInfo.storageUsage) /
                            Number(hostSystemInfo.storageCapacity)) *
                          100
                        ).toFixed(0)
                      ) || 0
                    "
...
::v-deep {
  ...
  // }
  // $thPrimaryColor: rgb(12, 49, 63);

  // .el-progress-bar__outer {
  //     background: linear-gradient(rgb(12, 96, 108), $thPrimaryColor);
  // }
}
