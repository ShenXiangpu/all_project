<template>
    <div class="app-container">
        <border-container class="marginBottom10" :lBCircle="lBCircle" :lSCircle="lSCircle"
            :rBCircle="rBCircle" :rSCircle="rSCircle" :height="15" :isShowTitle="false" :isBgShow="false">
            <template #content>
                <div style="padding: 10px 0 0px 10px;">
                    <el-form ref="queryFormRef" :model="queryParams" :inline="true">
                        <el-form-item label="菜单名称" prop="keyWord">
                            <el-input v-model="queryParams.keyWord" placeholder="请输入菜单名称" clearable
                    ...
                    <el-select v-model="queryParams.menuType" placeholder="请选择菜单类型">
                        ...
                        <el-form-item>
                            <el-button type="success" class="editSuccess" icon="el-icon-search" @click="handleQueryTop">搜索</el-button>
        ...
        <el-card>
            ...
            <el-table v-loading="loading" :data="menuList"
                ...
                <el-table-column label="创建时间" align="center" width="200" prop="createdAt">
                </el-table-column>

                <el-table-column label="修改时间" align="center" width="200" prop="updatedAt">
                </el-table-column>

                <el-table-column label="操作" fixed="right" align="center" min-width="240">
                    <template #default="scope">
                        <el-button size="small" type="success" class="editSuccess" v-if="scope.row.type != 'BUTTON'"
        ...
        <el-dialog :title="dialog.title" :visible.sync="dialog.visible" @close="cancel" width="750px">
            <el-form ref="dataFormRef" :model="formData" :rules="formDataRules" label-width="80px">
                <el-form-item label="上级菜单" prop="parentId">
                    <tree-select v-model="formData.parentId" :options="parentMenu" placeholder="选择上级菜单"
                ...
                <el-form-item label="路由路径" prop="path" v-if="formData.type == 'MENU' || formData.type == 'CATALOG'">
                ...
                <el-form-item v-if="formData.type == 'MENU'" label="页面路径" prop="component">
                    <el-input v-model="formData.component" placeholder="sysMng/roleMng">
                        <template v-if="formData.parentId != '0'" #prepend>@/views/</template>
                    </el-input>
                </el-form-item>
                ...
                <!-- 权限标识 -->
                <el-form-item v-if="formData.type == 'BUTTON'" label="权限标识" prop="url">
                ...
                <el-form-item label="菜单图标" v-if="formData.menuType !== 'BUTTON'">
                    <el-popover placement="bottom-start" width="460" trigger="click" @show="$refs['iconSelect'].reset()">
                        ...
                        <el-input slot="reference" v-model="formData.icon" clearable placeholder="点击选择图标">
                            <svg-icon v-if="formData.icon" slot="prefix" :icon-class="formData.icon" class="el-input__icon"
...
<script>
import { createMenu, deleteMenu, editMenu, getMenuList, getMenuDetailById } from '@/api/system';
    ...
    data() {
        return {
            ...
            formDataRules: {
                ...
                url: [
                    { required: true, message: '请输入按钮api', trigger: 'blur' },
                ],
                icon: [
                    { required: true, message: '请选择目录或者菜单图标', trigger: 'blur' },
                ],
                // status: [
        ...
        submitForm(e) {
            this.$refs.dataFormRef.validate(async (valid) => {
                if (valid) {
                    ...
                    const res = title == '添加' ? await createMenu(this.formData) : await editMenu(this.formData)
                    if (res && res.flag) {
