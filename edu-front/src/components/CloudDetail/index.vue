    ...
    <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
      <el-tab-pane label="云环境详情" name="first">
        <env-detail
          ...
          :resetTLoading="resetTLoading"
          :vmEnvDetail="vmEnvDetail"
          @handleDeskTopReOpen="handleDeskTopReOpen"
          @goToNovnc="goToNovnc"
          ...
          :commitHomeloading="commitHomeloading"
          @cancelHome="cancelHome"
          @submitFormHomeWork="submitFormHomeWork"
      ...
      <el-tab-pane label="监控" name="second" v-if="userRolesNames != '学生'">
        <monitor
          ...
          @queryPeriodPerformance="queryPeriodPerformance"
          :vmId="vmId"
          :cpuUsage="cpuUsage"
      ...
      >
        <data-snapshot
          :vmEnvDetail="snapshotVmEnvDetail"
          @rebackClose="rebackClose"
          :dialogRebackVisible="dialogRebackVisible"
          @handleReback="handleReback"
          @closeDilog="closeDilog"
          :dialogFormVisible="dialogFormVisible"
          @submit="submitUpdateSnapshot"
          @handleRetentionDaysChange="handleRetentionDaysChange"
          @querySnapshotList="queryPlicyByVmId"
          :ruleForm="ruleForm"
          @handleDelete="handleDeleteSnapshot"
          @handleDeleteAll="handleDeleteAllSnapshot"
          :dataSnapshotList="dataSnapshotList"
          @commitReback="commitReback"
          :rebackLoading="rebackLoading"
        ></data-snapshot>
      </el-tab-pane>
      <el-tab-pane
        label="告警策略"
        name="fourth"
        v-if="userRolesNames != '学生' && false"
      >
        <alarm-strategy
          ...
          :operators="operators"
          :alarmForm="alarmForm"
          :dialogAlarmVisible="dialogAlarmVisible"
          :icSupportAlarmTypes="icSupportAlarmTypes"
          @addAlarmStrategy="addAlarmStrategy"
          @changeType="changeType"
          @commitAlarmStrategy="commitAlarmStrategy"
          @handleDeleteAlarm="handleDeleteAlarm"
          @updataAlarm="updataAlarm"
          :alarmConfigsList="alarmConfigsList"
        ></alarm-strategy>
      </el-tab-pane>
      <el-tab-pane
        label="告警历史"
...
import { listFile, getHomeworks, submitHomework } from "@/api/edu/job";
...
export default {
  ...
  methods: {
      ...
      getVmInfoBySnapshotName({ name }).then((res) => {
        if (res && res.flag) {
          ...
          if (vmEnvDetail) {
            ...
            const snapshotArr =
              vmEnvDetail &&
              vmEnvDetail.snapshotName &&
              vmEnvDetail.snapshotName.split("-");
            const time =
              snapshotArr && snapshotArr.length > 0 && snapshotArr.pop();
    ...
    queryPeriodPerformance(obj) {
      let startTime = this.$moment(obj && obj.startTime).format(
        "YYYY-MM-DD HH:mm:ss"
      );
      let endTime = this.$moment(obj && obj.endTime).format(
        "YYYY-MM-DD HH:mm:ss"
      );
    ...
    async queryEnvDetail(id) {
      ...
      if (vmEnvDetail) {
        ...
        let architectureList = Object.keys(tools);
        vmEnvDetail.architectureList = architectureList;
        vmEnvDetail.tools = tools[architectureList];
        let userCountsToPwd =
          vmEnvDetail &&
          vmEnvDetail.userCountsToPwd &&
          Base64.decode(vmEnvDetail.userCountsToPwd);
