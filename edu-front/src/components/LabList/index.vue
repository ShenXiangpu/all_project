  ...
  <div>
    <el-card class="lab-card" v-loading="detailLoading">
      <div class="show-info" @click.stop="handleDetail">
        <div class="info-type" v-if="item.coverImageTrialTypeName">
          【{{ item.coverImageTrialTypeName }}】
        </div>
        <el-popover placement="top" trigger="hover" popper-class="popper-class">
          <div style="max-width: 400px; min-width: 50px" v-if="toolsShow">
            {{ toolsShow }}
          </div>
          <div slot="reference">
            <div class="info-tools view-text" v-if="toolsShow">
              工具：{{ toolsShow }}
            </div>
          </div>
        </el-popover>
        <el-popover
          v-if="item.coverImageLabName"
          placement="top"
          trigger="hover"
        >
          <div
      ...
      <div
        v-if="coverImageDisplayAuthor == '1' && isShowName"
        class="lab-card-mark show-text"
      >
...
export default {
  ...
  computed: {
    toolsShow() {
      let item = this.item;
      let tools = item && item.tools;
      let artList = tools && Object.keys(tools);
      let art = artList && artList.length > 0 && artList[0];
      let vendor = tools && tools[art];
      let vendor0 = vendor && vendor[0];
      let edaTools = vendor0 && vendor0.edaTools;
      const toolsName = edaTools && edaTools[0] && edaTools[0].toolName;
      return toolsName;
    },
  },
  created() {},
