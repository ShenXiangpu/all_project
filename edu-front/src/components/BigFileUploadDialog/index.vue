...
export default {
  props: {
    ...
    target: {
      default: "/edu/zkxy-vm-web/file/upload",
      type: String,
    },
    isDirectory: {
      default: false,
    ...
    return {
      ...
      options: {
        ...
        processParams: (params, file, chunk, isTest) => {
          console.log("params", params, file, chunk, isTest);
          return {
  ...
  methods: {
    ...
    onFileAdded(file, event) {
      ...
      if (file.size > MAX_FILE_SIZE) {
        this.$message.error("文件上传大小超过限制(2G)");
        console.log("uploader", this.$refs.uploader, file);
        this.$emit("handleClose");
        return false;
      } else {
        this.uploadFileList.push(file);
        console.log("上传文件列表2：" + this.uploadFileList);
...
::v-deep .uploader-file-actions {
  .uploader-file-pause,.uploader-file-resume {
    display: none !important;
  }
}
